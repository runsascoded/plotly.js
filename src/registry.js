...
export const subplotsRegistry = {};
export const componentsRegistry = {};
export const layoutArrayContainers = [];
export const layoutArrayRegexes = [];
export const traceLayoutAttributes = {};
export const localeRegistry = {};
export const apiMethodRegistry = {};
export let collectableSubplotTypes = null;
export function register(_modules) {
...
export function getModule(trace) {
    const _module = modules[getTraceType(trace)];
    if (!_module)
        return false;
    return _module._module;
}
export function traceIs(traceType, category) {
...
export function getComponentMethod(name, method) {
    const _module = componentsRegistry[name];
    if (!_module)
        return noop;
    return _module[method] || noop;
}
export function call(..._args) {
    const name = _args[0];
...
function registerLocale(_module) {
    ...
    if (baseLocale !== locale) {
        ...
        if (!baseLocaleObj)
            locales[baseLocale] = baseLocaleObj = {};
    ...
    if (hasDict)
        localeObj.dictionary = newDict;
    if (hasFormat)
        localeObj.format = newFormat;
}
...
function mergeComponentAttrsToTrace(componentName, traceType) {
    ...
    if (!componentSchema || !componentSchema.traces)
        return;
...
function mergeComponentAttrsToSubplot(componentName, subplotName) {
    const componentSchema = componentsRegistry[componentName].schema;
    if (!componentSchema || !componentSchema.subplots)
        return;
