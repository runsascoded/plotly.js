...
export function getDelay(fullLayout) {
    if (!fullLayout._has)
        return 0;
    return (fullLayout._has('gl3d') ||
        fullLayout._has('mapbox') ||
        fullLayout._has('map')) ? 500 : 0;
}
export function getRedrawFunc(gd) {
    return function () {
...
export function createBlob(url, format) {
    if (format === 'svg') {
