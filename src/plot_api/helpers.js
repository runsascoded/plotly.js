...
export function clearPromiseQueue(gd) {
...
export function cleanLayout(layout) {
    ...
    for (i = 0; i < keys.length; i++) {
        ...
        if (axisAttrRegex && axisAttrRegex.test(key)) {
            ...
            if (ax.overlaying)
                ax.overlaying = cleanId(ax.overlaying);
...
export function cleanData(data) {
    for (let tracei = 0; tracei < data.length; tracei++) {
        ...
        // axis ids x1 -> x, y1-> y
        if (trace.xaxis)
            trace.xaxis = cleanId(trace.xaxis, 'x');
        if (trace.yaxis)
            trace.yaxis = cleanId(trace.yaxis, 'y');
        ...
        if (_module && _module.colorbar) {
            ...
            if (container && container.colorscale) {
                if (container.colorscale === 'YIGnBu')
                    container.colorscale = 'YlGnBu';
                if (container.colorscale === 'YIOrRd')
                    container.colorscale = 'YlOrRd';
            }
        }
        ...
        if (emptyContainer(trace, 'line'))
            delete trace.line;
        if ('marker' in trace) {
            if (emptyContainer(trace.marker, 'line'))
                delete trace.marker.line;
            if (emptyContainer(trace, 'marker'))
                delete trace.marker;
        }
        // sanitize rgb(fractions) and rgba(fractions) that old tinycolor
        // supported, but new tinycolor does not because they're not valid css
...
function cleanTextPosition(textposition) {
    ...
    if (typeof textposition === 'string') {
        if (textposition.indexOf('top') !== -1)
            ...
            posY = 'bottom';
        if (textposition.indexOf('left') !== -1)
            posX = 'left';
        else if (textposition.indexOf('right') !== -1)
            posX = 'right';
    }
    return posY + ' ' + posX;
}
...
export function swapXYData(trace) {
    let i;
    ...
    if (Array.isArray(trace.z) && Array.isArray(trace.z[0])) {
        if (trace.transpose)
            ...
            trace.transpose = true;
    }
    if (trace.error_x && trace.error_y) {
        const errorY = trace.error_y;
        const copyYstyle = 'copy_ystyle' in errorY ? errorY.copy_ystyle : !(errorY.color || errorY.thickness || errorY.width);
    ...
    if (typeof trace.hoverinfo === 'string') {
        ...
        for (i = 0; i < hoverInfoParts.length; i++) {
            if (hoverInfoParts[i] === 'x')
                hoverInfoParts[i] = 'y';
            else if (hoverInfoParts[i] === 'y')
                hoverInfoParts[i] = 'x';
        }
        trace.hoverinfo = hoverInfoParts.join('+');
    }
}
export function coerceTraceIndices(gd, traceIndices) {
    if (isNumeric(traceIndices)) {
        return [traceIndices];
    }
    else if (!Array.isArray(traceIndices) || !traceIndices.length) {
        return gd.data.map((_, i) => i);
    }
...
export function manageArrayContainers(np, newVal, undoit) {
    const obj = np.obj;
...
function getParent(attr) {
    ...
    if (tail > 0)
        return attr.slice(0, tail);
}
export function hasParent(aobj, attr) {
    let attrParent = getParent(attr);
    while (attrParent) {
        if (attrParent in aobj)
            return true;
        attrParent = getParent(attrParent);
    }
    return false;
}
export function clearAxisTypes(gd, traces, layoutUpdate) {
        ...
        for (let j = 0; j < 3; j++) {
            ...
            if (ax && ax.type !== 'log') {
                ...
                if (sceneName.slice(0, 5) === 'scene') {
                    if (layoutUpdate[sceneName] !== undefined)
                        continue;
