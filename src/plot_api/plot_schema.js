...
import { extendDeepAll, isArrayOrTypedArray, isPlainObject, nestedProperty, valObjectMeta } from '../lib/index.js';
...
export function crawl(attrs, callback, specifiedLevel, attrString) {
    const level = specifiedLevel || 0;
    attrString = attrString || '';
    Object.keys(attrs).forEach((attrName) => {
        const attr = attrs[attrName];
        if (UNDERSCORE_ATTRS.indexOf(attrName) !== -1)
            return;
...
export function isValObject(obj) {
...
export function getTraceValObject(trace, parts) {
    const head = parts[0];
    ...
    if (!_module)
        _module = (modules[trace.type || baseAttributes.type.dflt] || {})._module;
    if (!_module)
        return false;
    ...
    if (!valObject)
        valObject = baseAttributes[head];
    return recurseIntoValObject(valObject, parts, i);
}
export function getLayoutValObject(fullLayout, parts) {
    const valObject = layoutHeadAttr(fullLayout, parts[0]);
...
function layoutHeadAttr(fullLayout, head) {
    ...
    if (basePlotModules) {
        ...
        for (i = 0; i < basePlotModules.length; i++) {
            ...
            if (_module.attrRegex && _module.attrRegex.test(head)) {
                ...
                if (_module.layoutAttrOverrides)
                    return _module.layoutAttrOverrides;
                // otherwise take the first attributes we find
                if (!out && _module.layoutAttributes)
                    out = _module.layoutAttributes;
            }
    ...
    if (head in baseLayoutAttributes)
        return baseLayoutAttributes[head];
    return false;
}
...
function getTraceAttributes(type) {
    ...
    crawl(copyModuleAttributes, function (attr, attrName, attrs, level, fullAttrString) {
        ...
        if (attr === undefined)
            nestedProperty(copyModuleAttributes, fullAttrString).set(undefined);
    });
