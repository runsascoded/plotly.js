...
export function makeTemplate(figure) {
...
function walkStyleKeys(parent, templateOut, getAttributeInfo, path, basePath) {
    ...
    for (const key in parent) {
        ...
        let attr = getAttributeInfo(nextBasePath);
        if (!attr) {
...
export function validateTemplate(figureIn, template) {
    const figure = Lib.extendDeep({}, {
        _context: dfltConfig,
        data: figureIn.data,
        ...
        for (const key in obj) {
            if (key.charAt(0) !== '_' && isPlainObject(obj[key])) {
                ...
                for (i = 0; i < paths.length; i++) {
                    ...
                    if (baseKey !== key)
                        nextPaths.push(getNextPath(obj, baseKey, paths[i]));
                }
