...
export function traceTemplater(dataTemplate) {
    ...
    function newTrace(traceIn) {
        ...
        const traceOut = { type: traceType, _template: null };
...
export function newContainer(container, name, baseName) {
    const template = container._template;
    let part = template && (template[name] || (baseName && template[baseName]));
    if (!isPlainObject(part))
        part = null;
    const out = container[name] = { _template: part };
    return out;
}
export function arrayTemplater(container, name, inclusionAttr) {
    const template = container._template;
    ...
    function newItem(itemIn) {
        ...
        const out = { name: itemIn.name, _input: itemIn };
...
export function arrayEditor(parentIn, containerStr, itemOut) {
