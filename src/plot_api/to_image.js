...
function toImage(gd, opts) {
    ...
    const clonedGd = document.createElement('div');
    clonedGd.style.position = 'absolute';
    clonedGd.style.left = '-5000px';
    document.body.appendChild(clonedGd);
    // extend layout with image options
    const layoutImage = Lib.extendFlat({}, layout);
    if (width) {
    ...
    function convert() {
        return new Promise(function (resolve, reject) {
            ...
            if (format === 'full-json') {
                let json = plots.graphJson(clonedGd, false, 'keepdata', 'object', true, true);
    ...
    return new Promise(function (resolve, reject) {
        plotApi.newPlot(clonedGd, data, layoutImage, configImage)
            ...
            .then(wait)
            .then(convert)
            .then((url) => { resolve(urlToImageData(url)); })
