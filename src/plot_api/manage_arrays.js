...
export function isAddVal(val) {
    return val === 'add' || isPlainObject(val);
}
export function isRemoveVal(val) {
    return val === null || val === 'remove';
}
export function applyContainerArrayChanges(gd, np, edits, flags, _nestedProperty) {
    ...
    if (edits['']) {
        ...
        const fullVal = edits[''][''];
        if (isRemoveVal(fullVal))
            np.set(null);
        else if (Array.isArray(fullVal))
            np.set(fullVal);
        else {
    ...
    for (i = deletes.length - 1; i >= 0; i--) {
        ...
        if (componentArrayFull)
            componentArrayFull.splice(deletes[i], 1);
    }
    if (!componentArray.length)
        np.set(null);
    else if (!componentArrayIn)
    ...
    if (drawOne !== noop) {
        ...
        if (firstIndexChange === -1) {
            ...
            for (i = 0; i < componentNums.length; i++) {
                componentNum = componentNums[i];
                if (componentNum >= firstIndexChange)
                    break;
