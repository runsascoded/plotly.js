...
function clearOutline(gd) { return getComponentMethod('selections', 'clearOutline')(gd); }
...
function _doPlot(gd, data, layout, config) {
    ...
    if (Array.isArray(data)) {
        ...
        if (graphWasEmpty)
            ...
            gd.data.push.apply(gd.data, data);
    ...
    function marginPushers() {
        ...
        if (gd._fullLayout.title.text && gd._fullLayout.title.automargin)
            allowAutoMargin(gd, 'title.automargin');
    ...
    function marginPushersAgain() {
        if (!didMarginChange(oldMargins, fullLayout._size))
            return;
    ...
    seq.push(timedDrawData, subroutines.finalDraw, initInteractions, addLinks, rehover, redrag, reselect);
    if (!deferAutoMargin) {
        seq.push(
    ...
    return plotDone.then(() => {
        ...
        if (deferAutoMargin) {
            requestAnimationFrame(function () {
                ...
                const deferredSeq = [timedMarginPushers, marginPushersAgain];
                if (hasCartesian)
                    deferredSeq.push(positionAndAutorange);
                deferredSeq.push(subroutines.layoutStyles);
                if (hasCartesian)
                    deferredSeq.push(drawAxes);
                deferredSeq.push(timedDrawData, subroutines.finalDraw, doAutoMargin);
                let deferredDone = syncOrAsync(deferredSeq, gd);
                if (!deferredDone || !deferredDone.then)
                    deferredDone = Promise.resolve();
                deferredDone.then(() => {
...
function setPlotContext(gd, config) {
    ...
    const szOut = (context._scrollZoom = {});
    if (szIn === true) {
        szOut.cartesian = 1;
        szOut.gl3d = 1;
        szOut.geo = 1;
...
function spliceTraces(gd, update, indices, maxPoints, updateArray) {
    ...
    for (let i = 0; i < updateProps.length; i++) {
        ...
        const out = updateArray(updateProps[i].target, updateProps[i].insert, maxp);
        prop.set(out[0]);
        // build the inverse update object for the undo operation
        if (!Array.isArray(undoUpdate[prop.astr]))
            undoUpdate[prop.astr] = [];
        undoUpdate[prop.astr].push(out[1]);
        // build the matching maxPoints undo object containing original trace lengths
        if (!Array.isArray(undoPoints[prop.astr]))
            undoPoints[prop.astr] = [];
 ...
 *
 */
function extendTraces(gd, update, indices, maxPoints, ...rest) {
    gd = getGraphDiv(gd);
...
function prependTraces(gd, update, indices, maxPoints, ...rest) {
    gd = getGraphDiv(gd);
    function updateArray(target, insert, maxp) {
        let newArray, remainder;
        if (isTypedArray(target)) {
...
function restyle(gd, astr, val, _traces) {
    ...
    if (Object.keys(aobj).length)
        gd.changed = true;
    const traces = helpers.coerceTraceIndices(gd, _traces);
    const specs = _restyle(gd, aobj, traces);
    const flags = specs.flags;
    // clear calcdata and/or axis types if required so they get regenerated
    if (flags.calc)
        gd.calcdata = undefined;
    if (flags.clearAxisTypes)
        helpers.clearAxisTypes(gd, traces, {});
    ...
    if (flags.fullReplot) {
        ...
        if (flags.style)
            seq.push(subroutines.doTraceStyle);
        if (flags.colorbars)
...
function makeNP(preGUI, guiEditFlag) {
    ...
    return function (container, attr, prefix) {
        const np = nestedProperty(container, attr);
        const npSet = np.set;
        np.set = function (val) {
...
function _restyle(gd, aobj, traces) {
    ...
    let i;
    // initialize flags
    const flags = editTypes.traceFlags();
    ...
    function getFullTrace(traceIndex) {
        ...
        for (let j = traceIndex; j < fullData.length; j++) {
            if (fullData[j]._input === data[traceIndex])
                return fullData[j];
        }
    ...
    function doextra(attr, val, i) {
        ...
        if (attr in aobj || helpers.hasParent(aobj, attr))
            return;
        let extraparam;
    ...
    for (let ai in aobj) {
        ...
        if (ai === 'autobinx' || ai === 'autobiny') {
            ...
            if (Array.isArray(vi))
                vi = vi.map(arrayBins(ai));
            else if (vi === false)
                vi = traces.map(allBins(ai));
            else
                vi = null;
        }
        redoit[ai] = vi;
...
function relayout(gd, astr, val) {
    ...
    if (flags.layoutReplot) {
        ...
        if (flags.legend)
            seq.push(subroutines.doLegend);
        if (flags.layoutstyle)
            seq.push(subroutines.layoutStyles);
        if (flags.axrange)
            addAxRangeSequence(seq, specs.rangesAltered);
        if (flags.ticks)
            seq.push(subroutines.doTicksRelayout);
        if (flags.modebar)
            seq.push(subroutines.doModeBar);
        if (flags.camera)
            seq.push(subroutines.doCamera);
        if (flags.colorbars)
            seq.push(subroutines.doColorBars);
        seq.push(emitAfterPlot);
    }
...
function axRangeSupplyDefaultsByPass(gd, flags, specs) {
    ...
    for (const k in flags) {
        if (k !== 'axrange' && flags[k])
            return false;
    }
    let axIn, axOut;
    const coerce = function (attr, dflt) {
...
function _relayout(gd, aobj) {
    ...
    for (i = 0; i < keys.length; i++) {
        if (keys[i].indexOf('allaxes') === 0) {
            for (j = 0; j < axes.length; j++) {
                ...
                if (!aobj[newkey])
                    aobj[newkey] = aobj[keys[i]];
            }
            delete aobj[keys[i]];
        }
    }
    // initialize flags
    const flags = editTypes.layoutFlags();
    // copies of the change (and previous values of anything affected)
    // for the undo / redo queue
    const redoit = {};
    const undoit = {};
    ...
    for (const ai in aobj) {
        ...
        const parentIn = nestedProperty(gd.layout, ptrunk).get();
        const parentFull = nestedProperty(fullLayout, ptrunk).get();
        const vOld = p.get();
        if (vi === undefined)
            continue;
        redoit[ai] = vi;
        ...
        const valObject = PlotSchema.getLayoutValObject(fullLayout, p.parts);
        ...
        if (containerArrayMatch) {
            ...
            if (!arrayEdits[arrayStr])
                arrayEdits[arrayStr] = {};
            let objEdits = arrayEdits[arrayStr][i];
            if (!objEdits)
                objEdits = arrayEdits[arrayStr][i] = {};
            objEdits[propStr] = vi;
            delete aobj[ai];
        }
            ...
            else if (fullLayout._has('scatter-like') &&
    ...
    for (arrayStr in arrayEdits) {
        const finished = manageArrays.applyContainerArrayChanges(gd, layoutNP(layout, arrayStr), arrayEdits[arrayStr], flags, layoutNP);
        if (!finished)
            flags.plot = true;
    }
...
function updateAutosize(gd) {
    ...
    if (gd.layout.autosize)
        plotAutoSize(gd, gd.layout, fullLayout);
...
function update(gd, traceUpdate, layoutUpdate, _traces) {
    gd = getGraphDiv(gd);
    helpers.clearPromiseQueue(gd);
    if (!isPlainObject(traceUpdate))
        traceUpdate = {};
    if (!isPlainObject(layoutUpdate))
        layoutUpdate = {};
    ...
    const relayoutFlags = relayoutSpecs.flags;
    // clear calcdata and/or axis types if required
    if (restyleFlags.calc || relayoutFlags.calc)
        gd.calcdata = undefined;
    if (restyleFlags.clearAxisTypes)
        helpers.clearAxisTypes(gd, traces, layoutUpdate);
    // fill in redraw sequence
    const seq = [];
    if (relayoutFlags.layoutReplot) {
        ...
        if (restyleFlags.style)
            seq.push(subroutines.doTraceStyle);
        if (restyleFlags.colorbars || relayoutFlags.colorbars)
            seq.push(subroutines.doColorBars);
        if (relayoutFlags.legend)
            seq.push(subroutines.doLegend);
        if (relayoutFlags.layoutstyle)
            seq.push(subroutines.layoutStyles);
        if (relayoutFlags.axrange)
            addAxRangeSequence(seq, relayoutSpecs.rangesAltered);
        if (relayoutFlags.ticks)
            seq.push(subroutines.doTicksRelayout);
        if (relayoutFlags.modebar)
            seq.push(subroutines.doModeBar);
        if (relayoutFlags.camera)
            seq.push(subroutines.doCamera);
        seq.push(emitAfterPlot);
    }
    seq.push(rehover, redrag, reselect);
...
function getFullTraceIndexFromUid(uid, fullData) {
    for (let i = 0; i < fullData.length; i++) {
        if (fullData[i]._fullInput.uid === uid)
            return i;
    }
    return -1;
}
...
function valsMatch(v1, v2) {
    ...
    if (v1IsObj || v1IsArray) {
        return (((v1IsObj && isPlainObject(v2)) || (v1IsArray && Array.isArray(v2))) &&
...
function applyUIRevisions(data, layout, oldFullData, oldFullLayout) {
    ...
    const newAutorangeIn = {};
    const newRangeAccepted = {};
                ...
                if (valsMatch(newVal, preGUIVal)) {
                    ...
                    if (preAuto || (preAuto === null && pre0 === null && pre1 === null)) {
                        ...
                        if (!(head in newAutorangeIn)) {
                            const newContainer = nestedProperty(layout, head).get();
                            newAutorangeIn[head] =
                                newContainer &&
...
function react(gd, data, layout, config) {
    ...
    if (!isPlotDiv(gd) || !oldFullData || !oldFullLayout) {
        ...
        if (configChanged) {
            ...
            const relayoutFlags = diffLayout(gd, oldFullLayout, newFullLayout, immutable, transition);
            ...
            //     fullLayout[ai] = gd._initialAutoSize[ai];
            // }
            if (updateAutosize(gd))
                relayoutFlags.layoutReplot = true;
    ...
    return plotDone.then(() => {
        if (!configChanged)
            gd.emit('plotly_react', { config, data, layout });
        return gd;
    });
}
function diffData(gd, oldFullData, newFullData, immutable, transition, newDataRevision) {
    ...
    function getTraceValObject(parts) {
        const out = PlotSchema.getTraceValObject(trace, parts);
...
function getDiffFlags(oldContainer, newContainer, outerparts, opts) {
    ...
    for (key in oldContainer) {
        ...
        if ((key === 'tick0' || key === 'dtick') && outerparts[0] !== 'geo') {
            ...
            if (tickMode === 'auto' || tickMode === 'array' || !tickMode)
                continue;
        }
        ...
        if (key === 'range' && newContainer.autorange)
            continue;
        if ((key === 'zmin' || key === 'zmax') && newContainer.type === 'contourcarpet')
            continue;
        valObject = getValObject(parts);
        // in case type changed, we may not even *have* a valObject.
        if (!valObject)
            continue;
        if (valObject._compareAsJSON && JSON.stringify(oldVal) === JSON.stringify(newVal))
            continue;
        ...
        if (wasArray && nowArray) {
            ...
            if (Array.isArray(oldValIn) && oldValIn === newValIn)
                continue;
        }
        if (newVal === undefined) {
            if (canBeDataArray && wasArray)
                flags.calc = true;
            else
                changed();
        }
            ...
            if (!inArray)
                flags.arrays[key] = arrayEditIndices;
...
function animate(gd, frameOrGroupNameOrFrameList, animationOpts) {
    gd = getGraphDiv(gd);
    if (!isPlotDiv(gd)) {
        throw new Error('This element is not a Plotly plot: ' +
            gd +
            ". It's likely that you've failed " +
            'to create a plot before animating it. For more details, see ' +
            'https://plotly.com/javascript/animations/');
    }
    const trans = gd._transitionData;
    ...
    const transitionOpts = animationOpts.transition;
    ...
    return new Promise(function (resolve, reject) {
        ...
        function queueFrames(frameList) {
            ...
            for (let i = 0; i < frameList.length; i++) {
                ...
                const transitionOpts = getTransitionOpts(i);
        ...
        function nextFrame() {
            ...
            if (newFrame) {
                ...
                plotsTransition(gd, newFrame.frame.data, newFrame.frame.layout, helpers.coerceTraceIndices(gd, newFrame.frame.traces), newFrame.frameOpts, newFrame.transitionOpts).then(() => {
...
function addFrames(gd, frameList, indices) {
    ...
    const insertions = [];
    const _frameHashLocal = {};
    for (i = frameList.length - 1; i >= 0; i--) {
        if (!isPlainObject(frameList[i]))
            continue;
        ...
        if (name &&
            ...
            warn('addFrames: overwriting frame "' +
            ...
            if (numericNameWarningCount === numericNameWarningCountLimit) {
                warn('addFrames: This API call has yielded too many of these warnings. ' +
    ...
    for (i = insertions.length - 1; i >= 0; i--) {
        ...
        if (typeof frame.name === 'number') {
            warn('Warning: addFrames accepts frames with numeric names, but the numbers are' +
...
function purge(gd) {
    ...
    if (fullLayout._container)
        fullLayout._container.remove();
...
function calcInverseTransform(gd) {
    ...
    if (equalDomRects(newBBox, fullLayout._lastBBox))
        return;
