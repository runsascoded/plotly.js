...
export function layoutStyles(gd) {
...
function lsInner(gd) {
    ...
    for (i = 0; i < axList.length; i++) {
        ax = axList[i];
        const counterAx = ax._anchorAxis;
        ...
        ax._mainMirrorPosition = (ax.mirror && counterAx) ?
            getLinePosition(ax, counterAx, alignmentConstants.OPPOSITE_SIDE[ax.side]) : null;
    }
    ...
    const noNeedForBg = (Color.opacity(fullLayout.paper_bgcolor) === 1 &&
    ...
    lowerBackgrounds.merge(lowerBgEnter).each(function (subplot) {
    ...
    for (i = 0; i < backgroundIds.length; i++) {
        ...
        if (plotinfo.bg && xa._offset !== undefined && ya._offset !== undefined) {
            plotinfo.bg
                .call(setRect, xa._offset - pad, ya._offset - pad, xa._length + 2 * pad, ya._length + 2 * pad)
    ...
    let xLinesXLeft, xLinesXRight, xLinesYBottom, xLinesYTop, leftYLineWidth, rightYLineWidth;
    let yLinesYBottom, yLinesYTop, yLinesXLeft, yLinesXRight, connectYBottom, connectYTop;
    let extraSubplot;
    ...
    function mainPath(ax, pathFn, pathFnFree) {
        if (!ax.showline || subplot !== ax._mainSubplot)
            return '';
        if (!ax._anchorAxis)
            return pathFnFree(ax._mainLinePosition);
        let out = pathFn(ax._mainLinePosition);
        if (ax.mirror)
            out += pathFn(ax._mainMirrorPosition);
        return out;
    }
    for (subplot in fullLayout._plots) {
        plotinfo = fullLayout._plots[subplot];
...
function shouldShowLineThisSide(ax, side, counterAx) {
    ...
    if (!counterAx.showline || !counterAx._lw)
        return false;
    // are we drawing *all* lines for counterAx?
    if (counterAx.mirror === 'all' || counterAx.mirror === 'allticks')
        return true;
...
export function drawMainTitle(gd) {
    const title = gd._fullLayout.title;
    const fullLayout = gd._fullLayout;
...
function needsMarginPush(gd, title, titleHeight) {
    ...
    let pushMargin = 0;
    if (title.yref === 'paper') {
        pushMargin = (titleHeight +
            title.pad.t +
            title.pad.b);
    }
    else if (title.yref === 'container') {
        pushMargin = (containerPushVal(position, titleY, titleYanchor, gd._fullLayout.height, titleHeight) +
            title.pad.t +
            title.pad.b);
    }
...
export function doTraceStyle(gd) {
    ...
    for (i = 0; i < calcdata.length; i++) {
        ...
        if (editStyle)
            editStyleCalls.push({ fn: editStyle, cd0: cd0 });
    }
...
export function doColorBars(gd) {
    getComponentMethod('colorbar', 'draw')(gd);
    return previousPromises(gd);
}
export function layoutReplot(gd) {
    const layout = gd.layout;
    gd.layout = undefined;
    return _doPlot(gd, '', layout);
}
export function doLegend(gd) {
    getComponentMethod('legend', 'draw')(gd);
    return previousPromises(gd);
}
export function doTicksRelayout(gd) {
    Axes.draw(gd, 'redraw');
...
export function redrawReglTraces(gd) {
    const fullLayout = gd._fullLayout;
...
export function doAutoRangeAndConstraints(gd) {
    const axList = Axes.list(gd, '', true);
    let ax;
    const autoRangeDone = {};
    for (let i = 0; i < axList.length; i++) {
...
export function drawMarginPushers(gd) {
    getComponentMethod('legend', 'draw')(gd);
    getComponentMethod('rangeselector', 'draw')(gd);
