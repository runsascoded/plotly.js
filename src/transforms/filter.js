...
export function supplyDefaults(transformIn) {
    const transformOut = {};
...
export function calcTransform(gd, trace, opts) {
    if (!opts.enabled)
        return;
    ...
    if (trace._length)
        len = Math.min(len, trace._length);
    ...
    if (typeof target === 'string') {
        ...
        if (attrTargetCalendar)
            targetCalendar = attrTargetCalendar;
    }
...
function getFilterFunc(opts, d2c, targetCalendar) {
    ...
    const d2cValue = (v) => { return d2c(v, 0, opts.valuecalendar); };
    const d2cTarget = (v) => { return d2c(v, 0, targetCalendar); };
    let coercedValue;
