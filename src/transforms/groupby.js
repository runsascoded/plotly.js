...
export function supplyDefaults(transformIn, traceOut, layout) {
    let i;
    const transformOut = {};
    ...
    if (styleIn) {
        for (i = 0; i < styleIn.length; i++) {
            ...
            if (Lib.isPlainObject(value))
                thisStyle.value = Lib.extendDeep({}, value);
            else if (value)
                delete thisStyle.value;
        }
    }
    return transformOut;
}
export function transform(data, state) {
    let newTraces, i, j;
    const newData = [];
