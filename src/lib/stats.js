...
export function aggNums(f, v, a, len) {
    let i, b;
    if (!len || len > a.length)
        len = a.length;
    if (!isNumeric(v))
...
export function len(data) {
    return aggNums(function (a) { return a + 1; }, 0, data);
}
export function mean(data, dataLen) {
    if (!dataLen)
        dataLen = len(data);
    return aggNums(function (a, b) { return a + b; }, 0, data) / dataLen;
}
export function geometricMean(data, dataLen) {
    if (!dataLen)
        dataLen = len(data);
    return Math.pow(aggNums(function (a, b) { return a * b; }, 1, data), 1 / dataLen);
}
export function midRange(numArr) {
    if (numArr === undefined || numArr.length === 0)
        return undefined;
    return (aggNums(Math.max, null, numArr) + aggNums(Math.min, null, numArr)) / 2;
}
export function variance(data, dataLen, dataMean) {
    if (!dataLen)
        dataLen = len(data);
    if (!isNumeric(dataMean))
        dataMean = mean(data, dataLen);
    return aggNums(function (a, b) {
        return a + Math.pow(b - dataMean, 2);
    }, 0, data) / dataLen;
}
export function stdev(data, dataLen, dataMean) {
