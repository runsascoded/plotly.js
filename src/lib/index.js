...
export const nestedProperty = _nestedProperty;
import _keyedContainer from './keyed_container.js';
export const keyedContainer = _keyedContainer;
import _relativeAttr from './relative_attr.js';
export const relativeAttr = _relativeAttr;
...
export const toLogRange = _toLogRange;
import _relinkPrivateKeys from './relink_private.js';
export const relinkPrivateKeys = _relinkPrivateKeys;
import _sortObjectKeys from './sort_object_keys.js';
export const sortObjectKeys = _sortObjectKeys;
import _clearResponsive from './clear_responsive.js';
export const clearResponsive = _clearResponsive;
import _preserveDrawingBuffer from './preserve_drawing_buffer.js';
export const preserveDrawingBuffer = _preserveDrawingBuffer;
import _makeTraceGroups from './make_trace_groups.js';
export const makeTraceGroups = _makeTraceGroups;
...
const MAX_SAFE = numConstants.FP_SAFE;
const MIN_SAFE = -MAX_SAFE;
const BADNUM = numConstants.BADNUM;
export function setAttrs(sel, attrs) {
    for (const k in attrs)
        sel.attr(k, attrs[k]);
    return sel;
}
export function adjustFormat(formatStr) {
    if (!formatStr || /^\d[.]\df/.test(formatStr) || /[.]\d%/.test(formatStr))
        return formatStr;
    if (formatStr === '0.f')
        return '~f';
    if (/^\d%/.test(formatStr))
        return '~%';
    if (/^\ds/.test(formatStr))
        return '~s';
    // try adding tilde to the start of format in order to trim
    if (!/^[~,.0$]/.test(formatStr) && /[&fps]/.test(formatStr))
        return '~' + formatStr;
    return formatStr;
}
const seenBadFormats = {};
...
export function constrain(v, v0, v1) {
    if (v0 > v1)
        return Math.max(v1, Math.min(v0, v));
...
export function smooth(arrayIn, FWHM) {
    ...
    for (i = 0; i < alen; i++) {
        ...
        for (j = 0; j < wlen; j++) {
            ...
            // multibounce
            if (k < -alen)
                k -= alen2 * Math.round(k / alen2);
            else if (k >= alen2)
                k -= alen2 * Math.floor(k / alen2);
            // single bounce
            if (k < 0)
                k = -1 - k;
            else if (k >= alen)
...
export function extractOption(calcPt, trace, calcKey, traceKey) {
    if (calcKey in calcPt)
        return calcPt[calcKey];
    ...
    if (!Array.isArray(traceVal))
        return traceVal;
}
function makePtIndex2PtNumber(indexToPoints) {
    const ptIndex2ptNumber = {};
...
export function minExtend(obj1, obj2, opt) {
    ...
    for (i = 0; i < keys.length; i++) {
        ...
        if (k.charAt(0) === '_' || typeof v === 'function')
            ...
            objOut[k] = minExtend(obj1[k], obj2[k], opt);
        else
            objOut[k] = v;
    }
...
export const isMacWKWebView = () => IS_MAC_WKWEBVIEW_REGEX.test(window.navigator.userAgent);
...
export function expandObjectPaths(data) {
    let match, key, prop, datum, idx, dest, trailingPath;
            ...
            if (data.hasOwnProperty(key)) {
                if ((match = key.match(dottedPropertyRegex))) {
                    ...
                    if (notValid(prop))
                        continue;
                    delete data[key];
                    data[prop] = extendDeepNoArrays(data[prop] || {}, objectFromPath(key, expandObjectPaths(datum))[prop]);
                }
...
export const hovertemplateString = (params) => templateFormatString({ ...params, opts: hovertemplateWarnings });
const texttemplateWarnings = {
    max: 10,
    count: 0,
    name: 'texttemplate'
};
export const texttemplateString = (params) => templateFormatString({ ...params, opts: texttemplateWarnings });
...
function multDivParser(inputStr) {
    ...
    if (match)
        return { key: match[1], op: match[2], number: Number(match[3]) };
...
export const texttemplateStringForShapes = (params) => templateFormatString({ ...params, opts: texttemplateWarningsForShapes });
...
function templateFormatString({ data = [], locale, fallback, labels = {}, opts, template }) {
    return template.replace(TEMPLATE_STRING_REGEX, (match, key, format) => {
        ...
        if (isSpaceOther || isSpaceOtherSpace)
            key = key.substring(1);
        if (isOtherSpace || isSpaceOtherSpace)
        ...
        if (value === undefined) {
            ...
            if (count === max)
                warn(`Too many '${name}' warnings - additional warnings will be suppressed.`);
            opts.count++;
...
export function subplotSort(a, b) {
    ...
    for (let i = 0; i < l; i++) {
        ...
        if (isNumA)
            numA = 10 * numA + charA - char0;
        if (isNumB)
            numB = 10 * numB + charB - char0;
        if (!isNumA || !isNumB) {
            if (numA !== numB)
                return numA - numB;
            if (charA !== charB)
...
export function pseudoRandom() {
    ...
    if (Math.abs(randSeed - lastVal) < 429496729)
        return pseudoRandom();
...
export function getTextTransform(transform) {
    ...
    if (!scale)
        ...
        scale = 1;
    return (strTranslate(targetX - scale * (textX + anchorX), targetY - scale * (textY + anchorY)) +
        strScale(scale) +
...
import { isArrayBuffer as _isArrayBuffer, isTypedArray as _isTypedArray, isArrayOrTypedArray as _isArrayOrTypedArray, isArray1D as _isArray1D, ensureArray as _ensureArray, concat as _concat, maxRowLength as _maxRowLength, minRowLength as _minRowLength, } from './array.js';
...
import { valObjectMeta as _valObjectMeta, coerce as _coerce, coerce2 as _coerce2, coerceFont as _coerceFont, coercePattern as _coercePattern, coerceHoverinfo as _coerceHoverinfo, coerceSelectionMarkerOpacity as _coerceSelectionMarkerOpacity, validate as _validate, } from './coerce.js';
...
import { dateTime2ms as _dateTime2ms, isDateTime as _isDateTime, ms2DateTime as _ms2DateTime, ms2DateTimeLocal as _ms2DateTimeLocal, cleanDate as _cleanDate, isJSDate as _isJSDate, formatDate as _formatDate, incrementMonth as _incrementMonth, dateTick0 as _dateTick0, dfltRange as _dfltRange, findExactDates as _findExactDates, MIN_MS as _MIN_MS, MAX_MS as _MAX_MS, } from './dates.js';
...
lib.MIN_MS = _MIN_MS;
lib.MAX_MS = _MAX_MS;
import { findBin as _findBin, sorterAsc as _sorterAsc, sorterDes as _sorterDes, distinctVals as _distinctVals, roundUp as _roundUp, sort as _sort, findIndexOfMin as _findIndexOfMin, } from './search.js';
...
import { aggNums as _aggNums, len as _len, mean as _mean, geometricMean as _geometricMean, median as _median, midRange as _midRange, variance as _variance, stdev as _stdev, interp as _interp, } from './stats.js';
...
lib.stdev = _stdev;
lib.interp = _interp;
import { init2dArray as _init2dArray, transposeRagged as _transposeRagged, dot as _dot, translationMatrix as _translationMatrix, rotationMatrix as _rotationMatrix, rotationXYMatrix as _rotationXYMatrix, apply3DTransform as _apply3DTransform, apply2DTransform as _apply2DTransform, apply2DTransform2 as _apply2DTransform2, convertCssMatrix as _convertCssMatrix, inverseTransformMatrix as _inverseTransformMatrix, } from './matrix.js';
...
import { deg2rad as _deg2rad, rad2deg as _rad2deg, angleDelta as _angleDelta, angleDist as _angleDist, isFullCircle as _isFullCircle, isAngleInsideSector as _isAngleInsideSector, isPtInsideSector as _isPtInsideSector, pathArc as _pathArc, pathSector as _pathSector, pathAnnulus as _pathAnnulus, } from './angles.js';
...
import { isLeftAnchor as _isLeftAnchor, isCenterAnchor as _isCenterAnchor, isRightAnchor as _isRightAnchor, isTopAnchor as _isTopAnchor, isMiddleAnchor as _isMiddleAnchor, isBottomAnchor as _isBottomAnchor, } from './anchor_utils.js';
...
import { segmentsIntersect as _segmentsIntersect, segmentDistance as _segmentDistance, getTextLocation as _getTextLocation, clearLocationCache as _clearLocationCache, getVisibleSegment as _getVisibleSegment, findPointOnPath as _findPointOnPath, } from './geometry2d.js';
...
import { extendFlat as _extendFlat, extendDeep as _extendDeep, extendDeepAll as _extendDeepAll, extendDeepNoArrays as _extendDeepNoArrays, } from './extend.js';
...
import { getGraphDiv as _getGraphDiv, isPlotDiv as _isPlotDiv, removeElement as _removeElement, addStyleRule as _addStyleRule, addRelatedStyleRule as _addRelatedStyleRule, deleteRelatedStyleRule as _deleteRelatedStyleRule, setStyleOnHover as _setStyleOnHover, getFullTransformMatrix as _getFullTransformMatrix, getElementTransformMatrix as _getElementTransformMatrix, getElementAndAncestors as _getElementAndAncestors, equalDomRects as _equalDomRects, } from './dom.js';
