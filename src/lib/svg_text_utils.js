...
// text converter
const FIND_TEX = /([^$]*)([$]+[^$]*[$]+)([^$]*)/;
export function convertToTspans(_context, gd, _callback) {
    ...
    if (tex) {
        ((gd && gd._promises) || []).push(new Promise(function (resolve) {
            ...
            const config = { fontSize: fontSize };
            texToSVG(tex[2], config, function (_svgEl, _glyphDefs, _svgBBox) {
                ...
                newSvg
                    ...
                    .attr('y', y);
                if (_callback)
                    _callback.call(_context, mathjaxGroup);
...
function texToSVG(_texString, _config, _callback) {
    const MathJaxVersion = parseInt((MathJax.version || '').split('.')[0]);
    if (MathJaxVersion !== 2 &&
        MathJaxVersion !== 3) {
        warn('No MathJax version:', MathJax.version);
        return;
    }
    let originalRenderer, originalConfig, originalProcessSectionDelay, tmpDiv;
    const setConfig2 = () => {
    ...
    const finalizeMathJax = () => {
        const sel = tmpDiv.select(MathJaxVersion === 2 ? '.MathJax_SVG' : '.MathJax');
 ...
 */
const PROTOCOLS = ['http:', 'https:', 'mailto:', '', undefined, ':'];
...
export function plainText(_str, opts) {
    opts = opts || {};
    const len = (opts.len !== undefined && opts.len !== -1) ? opts.len : Infinity;
    const allowedTags = opts.allowedTags !== undefined ? opts.allowedTags : ['br'];
    const ellipsis = '...';
...
function convertEntities(_str) {
    return _str.replace(ENTITY_MATCH, function (fullMatch, innerMatch) {
        ...
        if (innerMatch.charAt(0) === '#') {
            ...
            outChar = fromCodePoint(innerMatch.charAt(1) === 'x' ?
                parseInt(innerMatch.slice(2), 16) :
                parseInt(innerMatch.slice(1), 10));
        }
        else
            outChar = entityToUnicode[innerMatch];
...
function buildSVGText(containerNode, str) {
    ...
    function enterNode(nodeSpec) {
        ...
        if (type === 'a') {
            ...
            nodeType = 'tspan';
        if (nodeSpec.style)
            nodeAttrs.style = nodeSpec.style;
        ...
        for (const k in nodeAttrs)
            newNodeSel.attr(k, nodeAttrs[k]);
...
export function positionText(s, x, y) {
...
const onePx = '1px ';
export function makeTextShadow(color) {
    const x = onePx;
    const y = onePx;
    const b = onePx;
    return x + y + b + color + ', ' +
...
export function makeEditable(context, options) {
    const gd = options.gd;
    ...
    function appendEditable() {
        ...
        if (initialText === undefined)
            initialText = context.attr('data-unformatted');
        div.classed('plugin-editable editable', true)
            ...
            select(document).on('mouseup', function (event) {
                if (event.target === editDiv)
                    return false;
                if (document.activeElement === div.node())
                    div.node().blur();
            });
        })
    ...
    if (options.immediate)
        ...
        handlerElement.on('click', handleClick);
