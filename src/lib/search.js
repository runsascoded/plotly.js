...
export function findBin(val, bins, linelow) {
    if (isNumeric(bins.start)) {
        ...
        if (c > 90)
            loggers.log('Long binary search...');
        return n1 - 1;
    }
}
...
export function sorterAsc(a, b) { return a - b; }
export function sorterDes(a, b) { return b - a; }
export function distinctVals(valsIn) {
...
;
export function roundUp(val, arrayIn, reverse) {
    let low = 0;
...
export function sort(array, sortFn) {
    let notOrdered = 0;
    let notReversed = 0;
    for (let i = 1; i < array.length; i++) {
        ...
        if (pairOrder < 0)
            ...
            notReversed = 1;
        if (notOrdered && notReversed)
            return array.sort(sortFn);
    }
    return notReversed ? array : array.reverse();
}
export function findIndexOfMin(arr, fn) {
    fn = fn || identity;
    let min = Infinity;
    let ind;
