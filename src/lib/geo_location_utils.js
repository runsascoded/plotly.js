...
const countryIds = Object.keys(countryRegex);
const locationmodeToIdFinder = {
    'ISO-3': identity,
    'USA-states': identity,
...
function countryNameToISO3(countryName) {
    for (let i = 0; i < countryIds.length; i++) {
        ...
        if (regex.test(countryName.trim().toLowerCase()))
            return iso3;
    }
...
function getTraceGeojson(trace) {
    ...
    const PlotlyGeoAssets = window.PlotlyGeoAssets || {};
