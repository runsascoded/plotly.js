...
export const aggNums = function(f: (a: number, b: number) => number, v: any, a: any[], len?: number): any {
    let i: number,
        b: any[];
...
export const stdev = function(data: any[], dataLen?: number, dataMean?: number): number {
    return Math.sqrt(variance(data, dataLen, dataMean));
};

export const median = function(data: any[]): number {
    const b = data.slice().sort();
    return interp(b, 0.5);
};

export const interp = function(arr: any[], n: number): number {
