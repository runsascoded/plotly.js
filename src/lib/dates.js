...
export function dateTick0(calendar, dayOfWeek) {
...
export function dfltRange(calendar) {
    if (isWorldCalendar(calendar)) {
...
let MIN_MS, MAX_MS;
export function dateTime2ms(s, calendar) {
...
export function isDateTime(s, calendar) {
...
export function ms2DateTime(ms, r, calendar) {
    if (typeof ms !== 'number' || !(ms >= MIN_MS && ms <= MAX_MS))
        return BADNUM;
    if (!r)
        r = 0;
    const msecTenths = Math.floor(mod(ms + 0.05, 1) * 10);
    const msRounded = Math.round(ms - msecTenths / 10);
    let dateStr, h, m, s, msec10, d;
    if (isWorldCalendar(calendar)) {
...
export function ms2DateTimeLocal(ms) {
    if (!(ms >= MIN_MS + ONEDAY && ms <= MAX_MS - ONEDAY))
        return BADNUM;
    const msecTenths = Math.floor(mod(ms + 0.05, 1) * 10);
...
export function cleanDate(v, dflt, calendar) {
...
export function formatDate(x, fmt, tr, formatter, calendar, extraFormat) {
    calendar = isWorldCalendar(calendar) && calendar;
    if (!fmt) {
        if (tr === 'y')
            fmt = extraFormat.year;
        else if (tr === 'm')
            fmt = extraFormat.month;
        else if (tr === 'd') {
...
export function incrementMonth(ms, dMonth, calendar) {
    calendar = isWorldCalendar(calendar) && calendar;
    ...
    if (calendar) {
        try {
            ...
            if (dMonth % 12)
                calInstance.add(cDate, dMonth, 'm');
            else
                calInstance.add(cDate, dMonth / 12, 'y');
...
export function findExactDates(data, calendar) {
    let exactYears = 0;
    let exactMonths = 0;
    let exactDays = 0;
    let blankCount = 0;
    let d;
    let di;
    const calInstance = (isWorldCalendar(calendar) &&
