...
export { isArray1D };
export function ensureArray(out, n) {
...
export function decodeTypedArraySpec(vIn) {
    let out = [];
    ...
    const BYTES_PER_ELEMENT = T.BYTES_PER_ELEMENT;
    let buffer = v.bdata;
    if (!isArrayBuffer(buffer)) {
        buffer = b64decode(buffer);
    }
    const shape = v.shape === undefined ?
        // detect 1-d length
...
export function isTypedArraySpec(v) {
    return (isPlainObject(v) &&
        v.hasOwnProperty('dtype') && (typeof v.dtype === 'string') &&
        v.hasOwnProperty('bdata') && (typeof v.bdata === 'string' || isArrayBuffer(v.bdata)) &&
        (v.shape === undefined || (v.hasOwnProperty('shape') && (typeof v.shape === 'string' || typeof v.shape === 'number'))));
}
...
export function concat(...arrays) {
    ...
    let allArray = true;
    let totalLen = 0;
    let _constructor, arg0, i, argi, posi, leni, out, j;
...
export function maxRowLength(z) {
    return _rowLength(z, Math.max, 0);
}
export function minRowLength(z) {
