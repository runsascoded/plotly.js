...
export function manageCommandObserver(gd, container, commandList, onchange) {
    let ret = {};
    let enabled = true;
    ...
    ret.lookupTable = {};
    const binding = hasSimpleAPICommandBindings(gd, commandList, ret.lookupTable);
    if (container && container._commandObserver) {
        if (!binding) {
...
export function hasSimpleAPICommandBindings(gd, commandList, bindingsByValue) {
    let i;
...
export function executeAPICommand(gd, method, args) {
    if (method === 'skip')
    ...
    return _method.apply(null, allArgs).catch((err) => {
        warn('API call to Plotly.' + method + ' rejected.', err);
        return Promise.reject(err);
    });
}
export function computeAPICommandBindings(gd, method, args) {
    let bindings;
    if (!Array.isArray(args))
        args = [];
    switch (method) {
