...
proto.initializeGLPlot = function () {
    ...
    */
    ...
    const relayoutCallback = (scene) => {
        if (scene.fullSceneLayout.dragmode === false)
            return;
    ...
    if (scene.glplot.canvas) {
        ...
        scene.glplot.canvas.addEventListener('mousemove', function () {
            if (scene.fullSceneLayout.dragmode === false)
                return;
            if (scene.camera.mouseListener.buttons === 0)
                return;
            const update = makeUpdate();
...
proto.render = function () {
    ...
    if (lastPicked !== null) {
        ...
        if (!traceNow.hovertemplate && !isHoverinfoAll) {
            if (hoverinfoParts.indexOf('x') === -1)
                labels.xLabel = undefined;
            if (hoverinfoParts.indexOf('y') === -1)
                labels.yLabel = undefined;
            if (hoverinfoParts.indexOf('z') === -1)
                labels.zLabel = undefined;
            if (hoverinfoParts.indexOf('text') === -1)
                selection.textLabel = undefined;
            if (hoverinfoParts.indexOf('name') === -1)
                lastPicked.name = undefined;
        }
        let tx;
        const vectorTx = [];
...
proto.plot = function (sceneData, fullLayout, layout) {
    ...
    if (aspectmode === 'cube') {
        ...
        if (aspectmode === 'data') {
            ...
            if (Math.max.apply(null, axesScaleRatio) /
...
proto.isCameraChanged = function (layout) {
    ...
    if (cameraDataLastSave === undefined) {
        ...
        if (!cameraDataLastSave.projection || (cameraData.projection &&
...
proto.isAspectChanged = function (layout) {
    ...
    return (aspectDataLastSave === undefined || (aspectDataLastSave.x !== aspectData.x ||
...
proto.toImage = function (format) {
    ...
    if (!format)
        format = 'png';
    if (scene.staticMode)
        scene.container.appendChild(STATIC_CANVAS);
    // Force redraw
    scene.glplot.redraw();
    ...
    if (scene.staticMode)
        scene.container.removeChild(STATIC_CANVAS);
    return dataURL;
};
