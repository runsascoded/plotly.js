...
export function resize(gd) {
    ...
    const p = new Promise(function (resolve, reject) {
        ...
        if (gd._redrawTimer)
            clearTimeout(gd._redrawTimer);
        if (gd._resolveResize)
            resolveLastResize = gd._resolveResize;
...
export function addLinks(gd) {
    ...
    if (!gd._context.showLink && !gd._context.showSources)
        return;
    ...
    if (gd._context.showSources)
        gd._context.showSources(gd);
    // 'view in plotly' link for embedded plots
    if (gd._context.showLink)
...
export function sendDataToCloud(gd) {
    const baseUrl = (window.PLOTLYENV || {}).BASE_URL || gd._context.plotlyServerURL;
    if (!baseUrl)
        return undefined;
...
export function supplyDefaults(gd, opts) {
    ...
    const formatObj = getFormatObj(gd, d3FormatKeys);
    ...
    if (oldFullLayout._initialAutoSizeIsDone) {
        ...
        if (!newLayout.width)
            newFullLayout.width = oldWidth;
        if (!newLayout.height)
            newFullLayout.height = oldHeight;
        ...
        if (initialAutoSize)
            plotAutoSize(gd, newLayout, newFullLayout);
        else if (missingWidthOrHeight)
            sanitizeMargins(newFullLayout);
    ...
    for (i = 0; i < _modules.length; i++) {
        ...
        if (funci)
            pushUnique(crossTraceDefaultsFuncs, funci);
    }
    ...
    newFullLayout._hasOnlyLargeSploms = (newFullLayout._basePlotModules.length === 1 &&
    ...
    if (!newFullLayout._preGUI)
        newFullLayout._preGUI = {};
    // track trace GUI changes by uid rather than by trace index
    if (!newFullLayout._tracePreGUI)
        newFullLayout._tracePreGUI = {};
        ...
        if (cd0 && cd0.trace) {
            ...
            if (oldTrace._hasCalcTransform) {
                ...
                for (j = 0; j < arrayAttrs.length; j++) {
                    astr = arrayAttrs[j];
                    oldArrayVal = nestedProperty(oldTrace, astr).get().slice();
                    nestedProperty(newTrace, astr).set(oldArrayVal);
                }
            }
            cd0.trace = newTrace;
        }
...
function getTraceUids(oldFullData, newData) {
    ...
    for (i = 0; i < oldFullData.length; i++) {
        ...
        if (thisFullInput !== prevFullInput)
            oldFullInput.push(thisFullInput);
    ...
    for (i = 0; i < len; i++) {
        ...
        if (tryUid(newUid, i))
            continue;
        if (i < oldLen && tryUid(oldFullInput[i].uid, i))
            continue;
...
function emptySubplotLists() {
    let collectableSubplotTypes = _collectableSubplotTypes;
    const out = {};
    let i, j;
            ...
            if (subplotAttr) {
                ...
                if (Array.isArray(subplotAttr)) {
                    for (j = 0; j < subplotAttr.length; j++) {
                        pushUnique(collectableSubplotTypes, subplotAttr[j]);
                    }
                }
            }
        }
...
function getFormatter(formatObj, separators) {
    ...
    return {
        numberFormat: function (formatStr) {
            try {
                formatStr = formatLocale(formatObj).format(adjustFormat(formatStr));
            }
            catch (e) {
...
export function _hasPlotType(category) {
    ...
    for (i = 0; i < basePlotModules.length; i++) {
        if (basePlotModules[i].name === category)
            return true;
    }
    ...
    for (i = 0; i < modules.length; i++) {
        ...
        const _module = registryModules[name];
        if (_module && _module.categories[category])
            return true;
    }
    return false;
}
...
export function linkSubplots(newFullData, newFullLayout, oldFullData, oldFullLayout) {
    ...
    for (i = 0; i < ids.length; i++) {
        ...
        for (j = 0; j < newFullData.length; j++) {
            ...
            if (trace.xaxis === plotinfo.xaxis._id &&
    ...
    for (i = 0; i < axList.length; i++) {
        ...
        if (ax._counterAxes.length && ((ax.spikemode && ax.spikemode.indexOf('across') !== -1) ||
...
function findMainSubplot(ax, fullLayout) {
    ...
    if (!mainSubplotID || !fullLayout._plots[mainSubplotID]) {
        ...
        for (let j = 0; j < counterIDs.length; j++) {
            ...
            if (!nextBestMainSubplotID)
                nextBestMainSubplotID = id;
            const counterAx = axisIDs.getFromId(mockGd, counterPart);
...
export function supplyDataDefaults(dataIn, dataOut, layout, fullLayout) {
    ...
    function pushModule(fullTrace) {
        ...
        if (fullTrace.visible === true)
            pushUnique(visibleModules, _module);
        pushUnique(basePlotModules, fullTrace._module.basePlotModule);
        cnt++;
        ...
        if (fullTrace._input.visible !== false)
            colorCnt++;
    }
    const carpetIndex = {};
        ...
        if (subplotAttr && subplotAttrs) {
            ...
            if (Array.isArray(subplotAttr)) {
                for (i = 0; i < subplotAttr.length; i++) {
                    ...
                    if (subplots[attri])
                        pushUnique(subplots[attri], vali);
                    subplotId += vali;
                }
            }
            else {
            ...
            if (subplots[basePlotModule.name]) {
                pushUnique(subplots[basePlotModule.name], subplotId);
            }
        }
    }
    if (visible) {
        ...
        if (traceIs(traceOut, 'showLegend')) {
            Lib.coerce(traceIn, traceOut, _module.attributes.showlegend ? _module.attributes : attributes, 'showlegend');
            Lib.coerce(traceIn, traceOut, _module.attributes.legend ? _module.attributes : attributes, 'legend');
            coerce('legendwidth');
            coerce('legendgroup');
        ...
        if (!traceIs(traceOut, 'noHover')) {
            if (!traceOut.hovertemplate)
                coerceHoverinfo(traceIn, traceOut, layout);
...
export function supplyLayoutGlobalDefaults(layoutIn, layoutOut, formatObj) {
    ...
    coerceFont(coerce, 'title.subtitle.font', font, {
        overrideDflt: {
    ...
    if (layoutIn.width && layoutIn.height)
        sanitizeMargins(layoutOut);
    ...
    getComponentMethod('modebar', 'supplyLayoutDefaults')(layoutIn, layoutOut);
    getComponentMethod('shapes', 'supplyDrawNewShapeDefaults')(layoutIn, layoutOut, coerce);
    getComponentMethod('selections', 'supplyDrawNewSelectionDefaults')(layoutIn, layoutOut, coerce);
    coerce('meta');
    // do not include defaults in fullLayout when users do not set transition
    if (isPlainObject(layoutIn.transition)) {
        coerce('transition.duration');
        coerce('transition.easing');
        coerce('transition.ordering');
    }
    getComponentMethod('calendars', 'handleDefaults')(layoutIn, layoutOut, 'calendar');
    getComponentMethod('fx', 'supplyLayoutGlobalDefaults')(layoutIn, layoutOut, coerce);
...
export function plotAutoSize(gd, layout, fullLayout) {
    ...
    if (context.fillFrame) {
        ...
        const computedStyle = isPlot ? window.getComputedStyle(gd) : {};
    ...
    if (heightHasChanged || widthHasChanged) {
        if (widthHasChanged)
            fullLayout.width = newWidth;
        if (heightHasChanged)
            fullLayout.height = newHeight;
    }
...
function initMargins(fullLayout) {
    ...
    if (!fullLayout._pushmargin)
        fullLayout._pushmargin = {};
    if (!fullLayout._pushmarginIds)
        fullLayout._pushmarginIds = {};
    if (!fullLayout._reservedMargin)
        fullLayout._reservedMargin = {};
}
...
export function autoMargin(gd, id, o) {
    ...
    const minFinalWidth = constrain(width - margin.l - margin.r, MIN_SPECIFIED_WIDTH, minreducedwidth);
    const minFinalHeight = constrain(height - margin.t - margin.b, MIN_SPECIFIED_HEIGHT, minreducedheight);
...
function needsRedrawForShift(gd) {
    ...
    for (const ax in axList) {
        if (axList[ax].autoshift || axList[ax].shift)
            return true;
    }
    return false;
}
...
export function graphJson(gd, dataonly, mode, output, useDefaults, includeConfig) {
    ...
    const layout = (useDefaults) ? gd._fullLayout : gd.layout;
    ...
    function stripObj(d, keepFunction) {
        ...
        if ((dIsArray || dIsTypedArray) && d.dtype && d.shape) {
            ...
            return stripObj({
                ...
                bdata: 
                // case of ArrayBuffer
                isArrayBuffer(bdata) ? b64encode.encode(bdata) :
                    // case of b64 string
                    bdata
            }, keepFunction);
        }
        ...
        if (dIsTypedArray) {
            return simpleMap(d, identity);
        }
        // convert native dates to date strings...
        // mostly for external users exporting to plotly
        if (isJSDate(d))
            return ms2DateTimeLocal(+d);
        return d;
    }
    const obj = {
        data: (data || []).map((v) => {
    ...
    if (frames)
        obj.frames = stripObj(frames);
    if (includeConfig)
        obj.config = stripObj(gd._context, true);
...
export function computeFrame(gd, frameName) {
    ...
    while (framePtr.baseframe && (framePtr = frameLookup[framePtr.baseframe.toString()])) {
        ...
        if (frameNameStack.indexOf(framePtr.name) !== -1)
            break;
...
export function extendObjectWithContainers(dest, src, containerPaths) {
    ...
    if (containerPaths && containerPaths.length) {
        for (i = 0; i < containerPaths.length; i++) {
            containerProp = nestedProperty(expandedObj, containerPaths[i]);
            containerVal = containerProp.get();
            if (containerVal === undefined) {
                nestedProperty(containerObj, containerPaths[i]).set(null);
            }
            else {
                containerProp.set(null);
                nestedProperty(containerObj, containerPaths[i]).set(containerVal);
            }
        }
    }
    ...
    if (containerPaths && containerPaths.length) {
        for (i = 0; i < containerPaths.length; i++) {
            srcProp = nestedProperty(containerObj, containerPaths[i]);
            srcContainer = srcProp.get();
            if (!srcContainer)
                continue;
            destProp = nestedProperty(dest, containerPaths[i]);
...
export const layoutArrayContainers = registryLayoutArrayContainers;
/*
...
export function transition(gd, data, layout, traces, frameOpts, transitionOpts) {
    const opts = { redraw: frameOpts.redraw };
    const transitionedTraces = {};
    const axEdits = [];
    opts.prepareFn = function () {
        ...
        for (let i = 0; i < traceIndices.length; i++) {
            ...
            if (_module.animatable) {
                ...
                if (!transitionedTraces[n])
                    transitionedTraces[n] = [];
        ...
        supplyDefaults(gd);
        doCalcdata(gd);
        const newLayout = expandObjectPaths(layout);
        if (newLayout) {
            const subplots = gd._fullLayout._plots;
            for (const k in subplots) {
                const plotinfo = subplots[k];
                ...
                if (editX || editY) {
                    axEdits.push(extendFlat({ plotinfo: plotinfo }, editX, editY));
                }
            }
        }
        return Promise.resolve();
    ...
    opts.runFn = function (makeCallback) {
        ...
        for (const n in transitionedTraces) {
            ...
            const _module = gd._fullData[traceIndices[0]]._module;
...
export function transitionFromReact(gd, restyleFlags, relayoutFlags, oldFullLayout) {
    const fullLayout = gd._fullLayout;
    const transitionOpts = fullLayout.transition;
    const opts = {};
    const axEdits = [];
    opts.prepareFn = function () {
        ...
        opts.redraw = false;
        if (restyleFlags.anim === 'some')
            opts.redraw = true;
        if (relayoutFlags.anim === 'some')
            opts.redraw = true;
        for (const k in subplots) {
            const plotinfo = subplots[k];
            const xa = plotinfo.xaxis;
...
export function doCalcdata(gd, traces) {
    ...
    function transformCalci(i) {
        ...
        if (trace.visible === true && trace.transforms) {
            ...
            for (j = 0; j < trace.transforms.length; j++) {
                ...
                _module = undefined; // transformsRegistry was removed in v3
    ...
    function calci(i, isContainer) {
        trace = fullData[i];
        _module = trace._module;
        if (!!_module.isContainer !== isContainer)
            return;
        let cd = [];
...
function sortAxisCategoriesByValue(axList, gd) {
    ...
    function zMapCategory(type, ax, value) {
        ...
        if (type === 'histogram2dcontour') {
            ...
            return function (o, l) {
                if (o === 0 || l === 0)
                    return -1; // Skip first row and column
                if (xCategorical && o === value[l].length - 1)
                    return -1;
                if (yCategorical && l === value.length - 1)
                    return -1;
                ...
                for (k = 0; k < cd.length; k++) {
                    ...
                    if (isSplom) {
                        ...
                        if (catIndex === undefined)
                            catIndex = cdi[axLetter];
                        value = cdi.s;
                        if (value === undefined)
                            value = cdi.v;
                        if (value === undefined)
                            value = isX ? cdi.y : cdi.x;
