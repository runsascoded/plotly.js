...
proto.adjustLayout = function (ternaryLayout, graphSize) {
    ...
    _this.xaxis.isPtWithinRange = function (d) {
        return (d.a >= _this.aaxis.range[0] &&
    ...
    setClipUrl(_this.layers.frontplot, _this._hasClipOnAxisFalse ? null : _this.clipId, _this.graphDiv);
};
proto.drawAxes = function (doTitles) {
    ...
    _this.drawAx(caxis);
    if (doTitles) {
        const apad = Math.max(aaxis.showticklabels ? aaxis.tickfont.size / 2 : 0, (caxis.showticklabels ? caxis.tickfont.size * 0.75 : 0) +
    ...
    this.dragOptions = {
        ...
        prepFn: function (e, startX, startY) {
            ...
            if (freeMode(dragModeNow))
                _this.dragOptions.minDrag = 1;
            else
                _this.dragOptions.minDrag = undefined;
            if (dragModeNow === 'zoom') {
