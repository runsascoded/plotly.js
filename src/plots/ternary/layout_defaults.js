...
function handleTernaryDefaults(ternaryLayoutIn, ternaryLayoutOut, coerce, options) {
    ...
    if (aaxis.min + baxis.min + caxis.min >= sum) {
        ...
        baxis.min = 0;
        caxis.min = 0;
        if (ternaryLayoutIn.aaxis)
            delete ternaryLayoutIn.aaxis.min;
        if (ternaryLayoutIn.baxis)
            delete ternaryLayoutIn.baxis.min;
        if (ternaryLayoutIn.caxis)
            delete ternaryLayoutIn.caxis.min;
    }
}
function handleAxisDefaults(containerIn, containerOut, options, ternaryLayoutOut) {
    ...
    handleTickMarkDefaults(containerIn, containerOut, coerce, { outerTicks: true });
