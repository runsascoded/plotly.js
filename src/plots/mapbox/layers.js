...
const proto = MapboxLayer.prototype;
...
proto.needsNewImage = function (opts) {
    ...
    return (map.getSource(this.idSource) &&
...
proto.updateImage = function (opts) {
    ...
    const _below = this.findFollowingMapboxLayerId(this.lookupBelow());
    if (_below !== null) {
...
proto.updateSource = function (opts) {
    const map = this.subplot.map;
    if (map.getSource(this.idSource))
...
proto.updateLayer = function (opts) {
    ...
    const _below = this.findFollowingMapboxLayerId(below);
    this.removeLayer();
    if (isVisible(opts)) {
        subplot.addLayer({
            id: this.idLayer,
...
function convertSourceOpts(opts) {
    ...
    const source = opts.source;
    const sourceOpts = { type: sourceType };
    let field;
...
export default function createMapboxLayer(subplot, index, opts) {
    // @ts-ignore TS7009
    const mapboxLayer = new MapboxLayer(subplot, index);
