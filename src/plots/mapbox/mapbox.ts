...
import createMapboxLayer from './layers.js';
const drawMode = dragHelpers.drawMode;
const selectMode = dragHelpers.selectMode;

function Mapbox(this: any, gd: any, id: any) {
    this.id = id;
    this.gd = gd;
...
proto.createMap = function(calcData: any, fullLayout: any, resolve: any, reject: any) {
    ...
    const styleObj = self.styleObj = getStyleObj(opts.style, fullLayout);
    ...
    // create the map!
    const map = self.map = new mapboxgl.Map({
        container: self.div,
...
proto.updateMap = function(calcData: any, fullLayout: any, resolve: any, reject: any) {
    ...
    let promises: any[] = [];
    const styleObj = getStyleObj(opts.style, fullLayout);
...
function getStyleObj(val: any, fullLayout: any) {
    const styleObj: any = {};
    ...
    if(Lib.isPlainObject(val)) {
        ...
        } else if((constants.stylesNonMapbox as any)[val]) {
            styleObj.style = (constants.stylesNonMapbox as any)[val];
            const spec = styleObj.style.sources['plotly-' + val];
            const tiles = spec ? spec.tiles : undefined;
            if(
                tiles &&
                tiles[0] &&
