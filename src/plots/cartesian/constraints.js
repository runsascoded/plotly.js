...
export function handleDefaults(layoutIn, layoutOut, opts) {
            ...
            if (axOut.fixedrange) {
                for (const axId2 in group) {
                    ...
                    if ((layoutIn[axName2] || {}).fixedrange === false) {
                        warn('fixedrange was specified as false for axis ' +
...
function handleOneAxDefaults(axIn, axOut, opts) {
    ...
    for (i = 0; i < axIds.length; i++) {
        ...
        if (idi === thisID)
            continue;
        const axi = layoutOut[id2name(idi)];
    ...
    if (matches) {
        ...
        let matchRatio = extent(layoutOut, axOut) / extent(layoutOut, matchedAx);
...
export function enforce(gd) {
    ...
    for (i = 0; i < constraintGroups.length; i++) {
        ...
        for (j = 0; j < axisIDs.length; j++) {
            ...
            if (ax._inputDomain)
                ax.domain = ax._inputDomain.slice();
            else
                ax._inputDomain = ax.domain.slice();
            if (!ax._inputRange)
                ax._inputRange = ax.range.slice();
            ...
            if (ax.constrain === 'domain')
                hasAnyDomainConstraint = true;
        }
        // Do we have a constraint mismatch? Give a small buffer for rounding errors
        if (minScale > ALMOST_EQUAL * maxScale && !hasAnyDomainConstraint)
            continue;
...
export function getAxisGroup(fullLayout, axId) {
