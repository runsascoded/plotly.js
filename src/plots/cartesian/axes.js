...
axes.coerceRef = function (containerIn, containerOut, gd, attr, dflt, extraOption) {
    ...
    const attrDef = {};
    if (!dflt)
        dflt = axlist[0] || (typeof extraOption === 'string' ? extraOption : extraOption[0]);
    if (!extraOption)
        extraOption = dflt;
...
const getDataConversions = axes.getDataConversions = function (gd, trace, target, targetArray) {
    let ax;
    ...
    if (ax)
        return { d2c: ax.d2c, c2d: ax.c2d };
    // special case for 'ids'
    // -> cast to String
    if (d2cTarget === 'ids')
        return { d2c: toString, c2d: toString };
...
axes.saveRangeInitial = function (gd, overwrite) {
    ...
    for (let i = 0; i < axList.length; i++) {
        const ax = axList[i];
        const isNew = ax._rangeInitial0 === undefined &&
            ax._rangeInitial1 === undefined;
        const hasChanged = isNew || (ax.range[0] !== ax._rangeInitial0 ||
...
axes.autoBin = function (data, ax, nbins, is2d, calendar, size) {
    ...
    if (size) {
        ...
        if (nbins)
            ...
            const msexp = Math.pow(10, Math.floor(Math.log(distinctData.minDiff) / Math.LN10));
            const minSize = msexp * roundUp(distinctData.minDiff / msexp, [0.9, 1.9, 4.9, 9.9], true);
    ...
    let binStart = axes.tickIncrement(axes.tickFirst(dummyAx), finalSize, 'reverse', calendar);
    let binEnd, bincount;
    ...
    if (tickformat) {
        ...
        // %X: the locale’s time, such as %-I:%M:%S %p
            ...
            // %I: hour (12-hour clock) as a decimal number [01,12]
                ...
                if (noDtick && !isMDate && ax.dtick < ONEHOUR)
                    ax.dtick = ONEHOUR;
            }
            else if (/%p/.test(tickformat) // %p: either AM or PM
            ) {
                definedDelta = HALFDAY;
                if (noDtick && !isMDate && ax.dtick < HALFDAY)
                    ax.dtick = HALFDAY;
            }
            ...
            // %x: the locale’s date, such as %-m/%-d/%Y
                ...
                definedDelta = ONEDAY;
                if (noDtick && !isMDate && ax.dtick < ONEDAY)
                    ax.dtick = ONEDAY;
            }
            ...
            // %W: Monday-based week of the year as a decimal number [00,53]
                ...
                if (noDtick && !isMDate && ax.dtick < ONEWEEK)
                    ax.dtick = ONEWEEK;
            }
            ...
            // %m: month as a decimal number [01,12]
                ...
                if (noDtick && (isMDate ? nMonths(ax.dtick) < 1 : ax.dtick < ONEMINMONTH))
                    ax.dtick = 'M1';
            }
            ...
            // %q: quarter of the year as a decimal number [1,4]
                ...
                if (noDtick && (isMDate ? nMonths(ax.dtick) < 3 : ax.dtick < ONEMINQUARTER))
                    ax.dtick = 'M3';
            }
            ...
            // %y: year without century as a decimal number [00,99]
                ...
                if (noDtick && (isMDate ? nMonths(ax.dtick) < 12 : ax.dtick < ONEMINYEAR))
                    ax.dtick = 'M12';
            }
        }
    }
    ...
    for (let i = 0; i < tickVals.length; i++) {
        ...
        if (ax.rangebreaks && periodLength > 0) {
            ...
            for (let c = 0; c < nAll; c++) {
                const r = (c + 0.5) / nAll;
                if (ax.maskBreaks(v * (1 - r) + r * endPeriod) !== BADNUM)
                    n++;
            }
            periodLength *= n / nAll;
            if (!periodLength) {
            ...
            if (inBetween && actualDelta > ONEWEEK)
                periodLength = actualDelta; // center monthly & longer periods
        }
...
axes.calcTicks = function calcTicks(ax, opts) {
    ...
    for (let major = 1; major >= (hasMinor ? 0 : 1); major--) {
        ...
        const mockAx = major ? ax : extendFlat({}, ax, ax.minor);
        if (isMinor) {
        ...
        if (major) {
            ...
            majorId = Math.round((ax.r2l(x) -
                ax.r2l(ax.tick0)) / _dTick) - 1;
        }
        ...
        for (; axrev ?
            (x >= endTick) :
            (x <= endTick); x = axes.tickIncrement(x, dtick, axrev, calendar)) {
            if (major)
                majorId++;
            if (mockAx.rangebreaks) {
                if (!axrev) {
                    if (x < startTick)
                        continue;
                    if (mockAx.maskBreaks(x) === BADNUM && moveOutsideBreak(x, mockAx) >= maxRange)
                        break;
                }
            }
            // prevent infinite loops - no more than one tick per pixel,
            // and make sure each value is different from the previous
            if (tickVals.length > maxTicks || x === prevX)
                break;
            prevX = x;
            const obj = { value: x };
            if (major) {
    ...
    if (!ticklabelIndex) {
        ...
        allTickVals =
            allTickVals
                ...
                .filter((tick, index, self) => index === 0 || tick.value !== self[index - 1].value);
        const majorTickIndices = allTickVals
            .map((item, index) => item.minor === undefined && !item.skipLabel ? index : null)
    ...
    if (hasMinor) {
        const canOverlap = (ax.minor.ticks === 'inside' && ax.ticks === 'outside') ||
            (ax.minor.ticks === 'outside' && ax.ticks === 'inside');
        if (!canOverlap) {
            // remove duplicate minors
            const majorValues = tickVals.map((d) => d.value);
            const list = [];
    ...
    if (ax.rangebreaks) {
        ...
        for (i = tickVals.length - 1; i > -1; i--) {
            ...
            if (flip ?
                (prevL > l - fontSize) :
                (prevL < l + fontSize)) { // ensure one pixel minimum
    ...
    function setTickLabel(ax, tickVal) {
        ...
        if (p !== undefined) {
            ...
            if (p > maxRange || p < minRange) { // hide label if outside the range
                if (p > maxRange)
                    text.periodX = maxRange;
                if (p < minRange)
                    text.periodX = minRange;
                hideLabel(text);
            }
        }
        return text;
...
function filterRangeBreaks(ax, ticksOut) {
    if (ax.rangebreaks) {
        ...
        ticksOut = ticksOut.filter((d) => ax.maskBreaks(d.x) !== BADNUM);
    }
    return ticksOut;
}
function syncTicks(ax) {
    ...
    if (baseAxis._vals) {
        for (let i = 0; i < baseAxis._vals.length; i++) {
            ...
            const obj = axes.tickText(ax, vali);
            // assign minor ticks
...
function arrayTicks(ax, majorOnly) {
    ...
    for (let isMinor = 0; isMinor <= 1; isMinor++) {
        if ((majorOnly !== undefined) && ((majorOnly && isMinor) || (majorOnly === false && !isMinor)))
            continue;
        if (isMinor && !ax.minor)
            continue;
        const vals = !isMinor ? ax.tickvals : ax.minor.tickvals;
        let text = !isMinor ? ax.ticktext : [];
        if (!vals)
            continue;
        // without a text array, just format the given values as any other ticks
        // except with more precision to the numbers
        if (!isArrayOrTypedArray(text))
            text = [];
...
function autoTickRound(ax) {
    ...
    if (ax.type === 'date') {
        ...
        if (String(dtick).charAt(0) === 'M') {
            ...
            if (tick0len > 10 || tick0str.slice(5) !== '01-01')
                ax._tickround = 'd';
            // show the month unless ticks are full multiples of a year
            else
                ax._tickround = (+(dtick.slice(1)) % 12 === 0) ? 'y' : 'm';
        }
...
axes.tickText = function (ax, x, hover, noSuffixPrefix) {
    ...
    else if (axType === 'category')
        formatCategory(ax, out);
    else if (axType === 'multicategory')
        formatMultiCategory(ax, out, hover);
    else if (isAngular(ax))
        formatAngle(ax, out, hover, extraPrecision, hideexp);
    else
        formatLinear(ax, out, hover, extraPrecision, hideexp);
    ...
    if (ax.labelalias && ax.labelalias.hasOwnProperty(out.text)) {
        const t = ax.labelalias[out.text];
        if (typeof t === 'string')
            out.text = t;
    }
...
axes.hoverLabelText = function (ax, values, hoverformat) {
    if (hoverformat)
        ax = extendFlat({}, ax, { hoverformat: hoverformat });
...
function formatDate(ax, out, hover, extraPrecision) {
    ...
    if (extraPrecision) {
        ...
        if (isNumeric(tr))
            ...
            tr = { y: 'm', m: 'd', d: 'M', M: 'S', S: 4 }[tr];
    }
    let dateStr = Lib.formatDate(out.x, fmt, tr, ax._dateFormat, ax.calendar, ax._extraFormat);
    let headStr;
    ...
    if (headStr) {
        if (hover) {
            ...
            if (tr === 'd')
                ...
                dateStr = headStr + (dateStr ? ', ' + dateStr : '');
        }
        else {
            if (!ax._inCalcTicks ||
...
function shouldSwitchSIToPowerFormat(exponent, exponentFormat) {
    if (!isSIFormat(exponentFormat))
        return false;
    if (exponentFormat === 'SI extended' && beyondSIExtended(exponent))
        return true;
    if (exponentFormat !== 'SI extended' && beyondSI(exponent))
        return true;
    return false;
}
function numFormat(v, ax, fmtoverride, hover) {
    ...
    if (v < e) {
        ...
        if (tickRound === 0)
            ...
            if (dp)
                v = v.slice(0, dp + tickRound).replace(/\.?0+$/, '');
        }
    ...
    if (exponent && exponentFormat !== 'hide') {
        if (shouldSwitchSIToPowerFormat(exponent, exponentFormat))
            exponentFormat = 'power';
        let signedExponent;
        if (exponent < 0)
            signedExponent = MINUS_SIGN + -exponent;
...
axes.getTickFormat = function (ax) {
    ...
    function isProperStop(dtick, range, convert) {
        const convertFn = convert || function (x) { return x; };
...
axes.findSubplotsWithAxis = function (subplots, ax) {
    const axMatch = new RegExp((ax._id.charAt(0) === 'x') ? ('^' + ax._id + 'y') : (ax._id + '$'));
    const subplotsWithAx = [];
    for (let i = 0; i < subplots.length; i++) {
        const sp = subplots[i];
        if (axMatch.test(sp))
            subplotsWithAx.push(sp);
    }
...
axes.makeClipPaths = function (gd) {
    ...
    for (i = 0; i < xaList.length; i++) {
        ...
        for (j = 0; j < yaList.length; j++) {
            if (i === 0)
                clipList.push({ x: fullWidth, y: yaList[j] });
    ...
    axClips.exit().remove();
    axClips.merge(axClipsEnter).each(function (d) {
...
axes.draw = function (gd, arg, opts) {
    ...
    axList.map((axId) => {
        ...
        if (ax.tickmode === 'sync' && ax.overlaying) {
            const overlayingIndex = axList.findIndex(function (axis) { return axis === ax.overlaying; });
...
axes.drawOne = function (gd, ax, opts) {
    ...
    if (ax._tickAngles)
        ax._prevTickAngles = ax._tickAngles;
    ax._tickAngles = {};
    ...
    if (!fullLayout._hasOnlyLargeSploms) {
        ...
        for (i = 0; i < subplotsWithAx.length; i++) {
            ...
            if (finishedGrids[mainCounterID])
                continue;
    ...
    if (ax.mirror === 'allticks') {
        ...
        for (i = 0; i < tickSubplots.length; i++) {
            ...
            const spTickPath = axes.makeTickPath(ax, p0, isMinor ? majorTickSigns[0] : minorTickSigns[0], { minor: isMinor }) +
                axes.makeTickPath(ax, p1, isMinor ? majorTickSigns[1] : minorTickSigns[1], { minor: isMinor });
            axes.drawTicks(gd, ax, {
                vals: tickVals,
    ...
    if (ax.type === 'multicategory') {
        const pad = { x: 2, y: 10 }[axLetter];
        seq.push(function () {
            const bboxKey = { x: 'height', y: 'width' }[axLetter];
    ...
    seq.push(function () {
        ...
        const sMirror = OPPOSITE_SIDE[ax.side].charAt(0);
...
function filterPush(push, automargin) {
    ...
    const keepMargin = Object.keys(MARGIN_MAPPING).reduce((data, nextKey) => {
        if (automargin.indexOf(nextKey) !== -1) {
            MARGIN_MAPPING[nextKey].forEach((key) => { data[key] = 1; });
        }
        return data;
    }, {});
...
axes.getTickSigns = function (ax, minor) {
    ...
    const sideOpposite = { x: 'top', y: 'right' }[axLetter];
    const main = ax.side === sideOpposite ? 1 : -1;
    let out = [-1, 1, main, -main];
...
axes.makeTransTickLabelFn = function (ax) {
    ...
    if (standoff) {
        const side = ax.side;
        const standoffSign = ((labelsInside && (side === 'top' || side === 'left')) ||
            (labelsOutside && (side === 'bottom' || side === 'right'))) ? 1 : -1;
...
axes.makeLabelFns = function (ax, shift, angle) {
    ...
    const labelsOverTicks = (ticklabelposition === 'inside' && ax.ticks === 'inside') ||
...
axes.drawTicks = function (gd, ax, opts) {
    ...
    const vals = []
        ...
        // minor vals
        opts.vals.filter((d) => d.minor && !d.noTick) :
        [])
        .concat(ax.ticks ?
        // major vals
        opts.vals.filter((d) => !d.minor && !d.noTick) :
    ...
    const ticksEnter = ticks.enter().append('path')
        ...
        return crispRound(gd, d.minor ? ax.minor.tickwidth : ax.tickwidth, 1) + 'px';
    })
        .attr('d', opts.path)
...
axes.drawGrid = function (gd, ax, opts) {
    ...
    const minorVals = hasMinor ? opts.vals.filter((d) => d.minor) : [];
    let majorVals = ax.showgrid ? opts.vals.filter((d) => !d.minor) : [];
    ...
    for (let major = 1; major >= 0; major--) {
        ...
        grid.merge(gridEnter).attr('transform', opts.transFn)
            ...
            return dashStyle(d.minor ? ax.minor.griddash : ax.griddash, d.minor ? ax.minor.gridwidth : ax.gridwidth);
        })
            .style('stroke-width', function (d) {
...
axes.drawZeroLine = function (gd, ax, opts) {
    ...
    const zlEnter = zl.enter().append('path')
        ...
        opts.layer.selectAll('path').sort((da, db) => idSort(da.id, db.id));
    });
...
axes.drawLabels = function (gd, ax, opts) {
    ...
    const vals = opts.vals.filter((d) => d.text);
    ...
    function positionLabels(s, angle) {
        s.each(function (d) {
            ...
            if (mathjaxGroup.empty()) {
                ...
                const mjShift = mjWidth * { end: -0.5, start: 0.5 }[anchor];
    ...
    ax._adjustTickLabelsOverflow = function () {
        ...
        if (!ticklabeloverflow || ticklabeloverflow === 'allow')
            return;
            ...
            if (anchorAx && insideTicklabelposition(anchorAx)) {
                (partialOpts || [
                    ...
                    if (e.K === ZERO_PATH.K) {
                        ...
                        sel = mainPlotinfo[ax._id.charAt(0) + 'axislayer'];
                    sel.each(function () {
                        let w = select(this);
                        if (e.L)
                            w = w.selectAll(e.L);
                        w.each(function (d) {
                            const q = ax.l2p(isPeriodLabel ? getPosX(d) : d.x) + ax._offset;
                            const t = select(this);
                            if (q < ax['_visibleLabelMax_' + anchorAx._id] &&
    ...
    function fixLabelOverlaps() {
        ...
        if (vals.length && ax.autotickangles &&
            ...
            const autoTickAnglesRadians = ax.autotickangles.map(function (degrees) { return degrees * Math.PI / 180; });
            let angleRadians = autoTickAnglesRadians.find(function (angle) { return Math.abs(Math.cos(angle)) <= maxCos; });
            ...
            if (preventOverlapWithTick) {
                ...
                for (i = 0; i < lbbArray.length; i++) {
                    ...
                    if ((xbnd[0] !== null && (lbb.left - ax.l2p(xbnd[0])) < gap) ||
    ...
    if (anchorAx && (anchorAx.autorange || anchorAx.insiderange) &&
...
axes.clipEnds = function (ax, vals) {
    return vals.filter((d) => clipEnds(ax, d.x));
};
...
function anyCounterAxLineAtZero(gd, ax, counterAxis, rng) {
    ...
    const zeroPosition = ax._offset + (((Math.abs(rng[0]) < Math.abs(rng[1])) === (axLetter === 'x')) ?
        0 : ax._length);
    function lineNearZero(ax2, ..._rest) {
        if (!ax2.showline || !ax2.linewidth)
            return false;
    ...
    for (let i = 0; i < counterLetterAxes.length; i++) {
        ...
        if (counterAxis2._mainAxis === mainCounterAxis &&
...
function makeAxisGroups(gd, traces) {
    ...
    for (i = 0; i < traces.length; i++) {
        ...
        if (!xi || !yi)
            continue; // not a 2D cartesian trace?
...
function mergeAxisGroups(intoSet, fromSet) {
    for (let i = 0; i < fromSet.length; i++) {
        if (intoSet.indexOf(fromSet[i]) === -1)
            intoSet.push(fromSet[i]);
    }
}
function swapAxisGroup(gd, xIds, yIds) {
    ...
    for (i = 0; i < allAxKeys.length; i++) {
        ...
        const xVal = xFullAxes[0][keyi];
        const yVal = yFullAxes[0][keyi];
        let allEqual = true;
        ...
        for (j = 1; j < yFullAxes.length && allEqual; j++) {
            ...
            else if (yFullAxes[j][keyi] !== yVal)
                allEqual = false;
        }
        if (allEqual) {
            if (coerceLinearX)
                layout[xFullAxes[0]._name].type = 'linear';
            if (coerceLinearY)
                layout[yFullAxes[0]._name].type = 'linear';
...
function swapAxisAttrs(layout, key, xFullAxes, yFullAxes, dfltTitle) {
    ...
    const xVal = np(layout[xFullAxes[0]._name], key).get();
    const yVal = np(layout[yFullAxes[0]._name], key).get();
    let i;
    if (key === 'title') {
...
function periodCompatibleWithTickformat(period, tickformat) {
    return (/%f/.test(tickformat) ? period >= ONEMICROSEC :
