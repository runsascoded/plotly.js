...
function getAutoRange(gd, ax) {
    ...
    let axReverse = autorange === 'reversed' ||
...
function makePadFn(fullLayout, ax, max) {
    ...
    const anchorAxis = ax._anchorAxis || {};
...
function padInsideLabelsOnAnchorAxis(fullLayout, ax, max) {
    ...
    for (const subplot in fullLayout._plots) {
        ...
        if (ax._id !== plotinfo.xaxis._id && ax._id !== plotinfo.yaxis._id)
            continue;
        const anchorAxis = (isX ? plotinfo.yaxis : plotinfo.xaxis) || {};
        if ((anchorAxis.ticklabelposition || '').indexOf('inside') !== -1) {
            // increase padding to make more room for inside tick labels of the counter axis
            if ((!max && (anchorAxis.side === 'left' ||
                anchorAxis.side === 'bottom')) || (max && (anchorAxis.side === 'top' ||
...
function doAutoRange(gd, ax, presetRange) {
    ...
    if (anchorAx && anchorAx.rangeslider) {
        const axeRangeOpts = anchorAx.rangeslider[ax._name];
        if (axeRangeOpts) {
...
function applyAutorangeOptions(range, ax) {
    ...
    if (include !== undefined) {
        let lMin = ax.d2l(min);
        let lMax = ax.d2l(max);
        if (!isArrayOrTypedArray(include))
            include = [include];
