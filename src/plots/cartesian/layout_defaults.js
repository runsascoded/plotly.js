...
export default function supplyLayoutDefaults(layoutIn, layoutOut, fullData) {
    ...
    for (i = 0; i < fullData.length; i++) {
        ...
        if (trace.type === 'funnel') {
            if (trace.orientation === 'h') {
                if (xaName)
                    xaMayHide[xaName] = true;
                if (yaName)
                    yaMayReverse[yaName] = true;
            }
            else {
                if (yaName)
                    yaMayHide[yaName] = true;
            }
        }
        else if (trace.type === 'image') {
            if (yaName)
                axHasImage[yaName] = true;
            if (xaName)
                axHasImage[xaName] = true;
        }
        else {
            if (yaName) {
    ...
    const missingMatchedAxisIdsLookup = {};
    ...
    for (i = 0; i < axNames.length; i++) {
        ...
        newAxLayoutOut();
        const visibleDflt = (axLetter === 'x' && !xaMustDisplay[axName] && xaMayHide[axName]) ||
            (axLetter === 'y' && !yaMustDisplay[axName] && yaMayHide[axName]);
        const reverseDflt = (axLetter === 'y' &&
