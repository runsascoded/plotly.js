...
export default function autoType(array, calendar, opts) {
    ...
    if (isArrayOrTypedArray(a) && !a.length)
        return '-';
    if (!noMultiCategory && multiCategory(a))
    ...
    if (category(a, convertNumeric))
        return 'category';
    if (linearOK(a, convertNumeric))
        return 'linear';
    return '-';
}
...
function linearOK(a, convertNumeric) {
    const len = a.length;
    for (let i = 0; i < len; i++) {
        if (hasTypeNumber(a[i], convertNumeric))
            return true;
    }
    return false;
}
...
function category(a, convertNumeric) {
    ...
    for (let f = 0; f < len; f += inc) {
        ...
        if (t === 'boolean')
            cats++;
        else if (convertNumeric ? cleanNumber(ai) !== BADNUM : t === 'number')
            nums++;
        else if (t === 'string')
            cats++;
    }
