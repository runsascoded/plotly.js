...
export function finalizeSubplots(layoutIn, layoutOut) {
    ...
    for (i = 0; i < xList.length; i++) {
        ...
        if (!allX[xi]) {
            ...
            if (!constants.idRegex.y.test(yi))
                yi = 'y';
            spSVG.push(xi + yi);
            spAll.push(xi + yi);
            if (!allY[yi]) {
    ...
    for (i = 0; i < yList.length; i++) {
        ...
        if (!allY[yi]) {
            ...
            if (!constants.idRegex.x.test(xi))
                xi = 'x';
            spSVG.push(xi + yi);
            spAll.push(xi + yi);
            if (!allX[xi]) {
    ...
    for (let z = 0; z < zindices.length; z++) {
        ...
        for (i = 0; i < subplots.length; i++) {
            ...
            if (z > 0) {
                ...
                if (idWithZ.indexOf(zindexSeparator) !== -1)
                    continue;
...
export function clean(newFullData, newFullLayout, oldFullData, oldFullLayout) {
    ...
    const oldSubplotList = oldFullLayout._subplots || {};
    let plotinfo;
    let i, k;
    ...
    if (hadCartesian && !hasCartesian) {
        ...
        for (i = 0; i < oldSubplotList.cartesian.length; i++) {
            ...
            if (oldSubplotId.indexOf(zindexSeparator) !== -1)
                continue;
...
export function drawFramework(gd) {
    ...
    subplotLayersMerged.each(function (d) {
        const id = d[0];
...
export function rangePlot(gd, plotinfo, cdSubplot) {
    makeSubplotLayer(gd, plotinfo);
