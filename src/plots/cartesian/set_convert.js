...
export default function setConvert(ax, fullLayout) {
    ...
    if (ax.rangebreaks) {
        ...
        l2p = function (v) {
            ...
            const b2 = ax._B[q] || 0;
            if (!isFinite(b2))
                return 0; // avoid NaN translate e.g. in positionLabels if one keep zooming exactly into a break
        ...
        p2l = function (px) {
            ...
            let q = 0;
            for (let i = 0; i < len; i++) {
                if (px < ax._rangebreaks[i].pmin)
                    break;
                if (px > ax._rangebreaks[i].pmax)
                    q = i + 1;
            }
    ...
    if (['linear', '-'].indexOf(ax.type) !== -1) {
        ...
        ax.p2r = p2l;
        ax.cleanPos = function (v) {
            if (Array.isArray(v) || (typeof v === 'string' && v !== ''))
                return v;
            return ensureNumber(v);
        };
        ax.setupMultiCategory = function (fullData) {
            ...
            if (group && ax._categories.length === 0) {
                for (const axId2 in group) {
                    if (axId2 !== axId) {
                        const ax2 = fullLayout[axisIds.id2name(axId2)];
    ...
    ax.limitRange = function (rangeAttr) {
        ...
        if (!rangeAttr)
            rangeAttr = 'range';
        const range = nestedProperty(ax, rangeAttr).get();
        ...
        if (range[0] === range[1]) {
            ...
            if (minallowed !== undefined) {
                ...
                if (maxallowed !== undefined)
                    _max = Math.min(_max, maxL);
                range[axrev ? 1 : 0] = _max;
            }
            if (maxallowed !== undefined) {
                let _min = maxL + 1;
                if (minallowed !== undefined)
                    _min = Math.max(_min, minL);
    ...
    ax._cleanRange = function (rangeAttr, opts) {
        ...
        let i, dflt;
        if (ax.type === 'date')
            dflt = dfltRange(ax.calendar);
        else if (axLetter === 'y')
            dflt = constants.DFLTRANGEY;
        else if (ax._name === 'realaxis')
            dflt = [0, 1];
        else
            dflt = opts.dfltRange || constants.DFLTRANGEX;
        ...
        for (i = 0; i < 2; i++) {
            if (ax.type === 'date') {
                ...
                if (ax.r2l(range[0]) === ax.r2l(range[1])) {
                    // split by +/- 1 second
                    const linCenter = constrain(ax.r2l(range[0]), MIN_MS + 1000, MAX_MS - 1000);
    ...
    ax.setScale = function (usePrivateRange) {
        ...
        if (ax.overlaying) {
            const ax2 = axisIds.getFromId({ _fullLayout: fullLayout }, ax.overlaying);
        ...
        if (ax.rangebreaks) {
            ...
            if (ax._rangebreaks.length) {
                ...
                for (i = 0; i < ax._rangebreaks.length; i++) {
                    brk = ax._rangebreaks[i];
                    ax._B.push(ax._B[ax._B.length - 1] -
    ...
    ax.locateBreaks = function (r0, r1) {
        ...
        const rangebreaksIn = ax.rangebreaks.slice().sort((a, b) => {
            if (a.pattern === WEEKDAY_PATTERN && b.pattern === HOUR_PATTERN)
                return -1;
            if (b.pattern === WEEKDAY_PATTERN && a.pattern === HOUR_PATTERN)
                return 1;
            return 0;
        });
        const addBreak = (min, max) => {
                ...
                if (brk.bounds) {
                    ...
                    switch (brk.pattern) {
                        case WEEKDAY_PATTERN:
                            ...
                            bndDelta = ((b1 < b0 ? 7 : 0) +
                                (b1 - b0)) * ONEDAY;
                            t0 += b0 * ONEDAY - (t0Date.getUTCDay() * ONEDAY +
                        ...
                        case HOUR_PATTERN:
                            ...
                            bndDelta = ((b1 < b0 ? 24 : 0) +
                                (b1 - b0)) * ONEHOUR;
                            t0 += b0 * ONEHOUR - (t0Date.getUTCHours() * ONEHOUR +
                                t0Date.getUTCMinutes() * ONEMIN +
                                t0Date.getUTCSeconds() * ONESEC +
                                t0Date.getUTCMilliseconds());
                            break;
    ...
    ax.getAutorangeDflt = function (range, options) {
        let autorangeDflt = !ax.isValidRange(range, 'nullOk');
        if (autorangeDflt && options && options.reverseDflt)
