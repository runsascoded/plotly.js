...
export function id2name(id) {
    if (typeof id !== 'string' || !id.match(constants.AX_ID_PATTERN))
        return;
...
export function name2id(name) {
    if (!name.match(constants.AX_NAME_PATTERN))
        return;
...
export function cleanId(id, axLetter, domainId) {
    const domainTest = /( domain)$/.test(id);
    if (typeof id !== 'string' || !id.match(constants.AX_ID_PATTERN))
        return;
    if (axLetter && id.charAt(0) !== axLetter)
        return;
    if (domainTest && (!domainId))
        return;
...
export function list(gd, axLetter, only2d) {
    ...
    if (!only2d) {
        ...
        for (i = 0; i < sceneIds3D.length; i++) {
            ...
            if (axLetter)
                out.push(scene[axLetter + 'axis']);
            else
                out.push(scene.xaxis, scene.yaxis, scene.zaxis);
        }
    }
    return out;
}
export function listIds(gd, axLetter) {
    const fullLayout = gd._fullLayout;
    if (!fullLayout)
        return [];
    const subplotLists = fullLayout._subplots;
    if (axLetter)
        return subplotLists[axLetter + 'axis'];
    return subplotLists.xaxis.concat(subplotLists.yaxis);
}
export function getFromId(gd, id, type) {
    const fullLayout = gd._fullLayout;
...
export function getFromTrace(gd, fullTrace, type) {
    const fullLayout = gd._fullLayout;
    let ax = null;
...
export function idSort(id1, id2) {
...
export function isLinked(fullLayout, axId) {
    return (isFound(axId, fullLayout._axisMatchGroups) ||
