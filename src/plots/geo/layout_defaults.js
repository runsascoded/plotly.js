...
function handleGeoDefaults(geoLayoutIn, geoLayoutOut, coerce, opts) {
    ...
    const traceIndices = subplotData.map((t) => t.index);
    ...
    if (geoLayoutIn.visible === false) {
        ...
        if (newTemplate.lonaxis)
            newTemplate.lonaxis.showgrid = false;
        if (newTemplate.lataxis)
            newTemplate.lataxis.showgrid = false;
        // set ref to copy
    ...
    for (let i = 0; i < axesNames.length; i++) {
        ...
        if (isScoped) {
            ...
            const rot = coerce('projection.rotation.' + axisName.slice(0, 3), scopeParams.projRotate[i]);
