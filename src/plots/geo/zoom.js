...
function zoomNonClipped(geo, projection) {
    ...
    let mouse0, rotate0, translate0, lastRotate, zoomPoint, mouse1, rotate1, point1, didZoom;
...
function zoomClipped(geo, projection) {
    const view = { r: projection.rotate(), k: projection.scale() };
    const zoom = initZoom(geo, projection);
    const event = d3eventDispatch(zoom, 'zoomstart', 'zoom', 'zoomend');
    let zooming = 0;
    const zoomOn = zoom.on;
    let zoomPoint;
    zoom.on('zoomstart', function (event, ...zoomstartArgs) {
        select(this).style(zoomstartStyle);
        ...
        zoomOn.call(zoom, 'zoom', function (event, ...args) {
        ...
        .on('zoomend', function (event, ...zoomendArgs) {
        select(this).style(zoomendStyle);
...
function unRoll(rotateAngles, pt, lastRotate) {
    ...
    if (dist1 <= dist2)
        return [newYaw1, newPitch1, lastRotate[2]];
    else
        return [newYaw2, newPitch2, lastRotate[2]];
}
...
function d3eventDispatch(target, ...restArgs) {
    const argumentz = restArgs.slice();
    const _dispatch = dispatch.apply(null, argumentz);
    _dispatch.of = function (thiz, argumentz) {
        return function (e1) {
            let e0;
            try {
