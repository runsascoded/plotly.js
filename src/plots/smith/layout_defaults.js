...
const makeImagDflt = memoize(function (realTickvals) {
    ...
    if (Lib.isTypedArray(realTickvals))
        realTickvals = Array.from(realTickvals);
    return realTickvals.slice().reverse().map((x) => -x)
        .concat([0])
        .concat(realTickvals);
}, String);
function handleDefaults(contIn, contOut, coerce, opts) {
    ...
    for (let i = 0; i < axisNames.length; i++) {
        ...
        axOut._traceIndices = subplotData.map((t) => t.index);
        ...
        if (visible) {
            ...
            if (isRealAxis) {
                ...
                const imagTickvalsDflt = makeImagDflt(contOut.realaxis.tickvals ||
                    layoutAttributes.realaxis.tickvals.dflt);
                coerceAxis('tickvals', imagTickvalsDflt);
            }
            // TODO: handle this case outside supply defaults step
            if (Lib.isTypedArray(axOut.tickvals))
                axOut.tickvals = Array.from(axOut.tickvals);
            let dfltColor;
            let dfltFontColor;
            let dfltFontSize;
            let dfltFontFamily;
