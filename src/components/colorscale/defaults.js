...
export default function colorScaleDefaults(parentContIn, parentContOut, layout, coerce, opts) {
    ...
    if (inTrace) {
        ...
        if (colorAx) {
            const colorbarVisuals = (traceIs(parentContOut, 'contour') &&
                nestedProperty(parentContOut, 'contours.coloring').get()) || 'heatmap';
    ...
    if (sclIn !== undefined)
        autoColorscaleDflt = !isValidScale(sclIn);
    if (sclTemplate !== undefined)
        autoColorscaleDflt = !isValidScale(sclTemplate);
    ...
    if (prefix !== 'marker.line.') {
        let showScaleDflt;
        if (prefix && inTrace)
            showScaleDflt = hasColorbar(containerIn);
        const showScale = coerce(prefix + 'showscale', showScaleDflt);
        if (showScale) {
            if (prefix && template)
                containerOut._template = template;
