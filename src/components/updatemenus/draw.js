...
export default function draw(gd) {
    ...
    const gButton = Lib.ensureSingle(menusMerged, 'g', constants.dropdownButtonGroupClassName, function (s) {
    ...
    headerGroupsJoin.exit().each(function (menuOpts) {
        gButton.call(removeAllButtons);
...
function drawHeader(gd, gHeader, gButton, scrollBox, menuOpts) {
    const header = Lib.ensureSingle(gHeader, 'g', constants.headerClassName, function (s) {
        s.style('pointer-events', 'all');
    });
    ...
    const arrow = Lib.ensureSingle(gHeader, 'text', constants.headerArrowClassName, function (s) {
...
function drawItemRect(item, menuOpts) {
    const rect = Lib.ensureSingle(item, 'rect', constants.itemRectClassName, function (s) {
        s
...
function drawItemText(item, menuOpts, itemOpts, gd) {
    const text = Lib.ensureSingle(item, 'text', constants.itemTextClassName, function (s) {
        s
...
function findDimensions(gd, menuOpts) {
    ...
    fakeButtonsJoin.merge(fakeButtonsEnter).each(function (buttonOpts, i) {
        const button = select(this);
