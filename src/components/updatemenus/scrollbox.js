...
ScrollBox.prototype.enable = function enable(position, translateX, translateY) {
    ...
    const hbarL = l;
    let hbarT = t + h;
    if (hbarT + hbarH > fullHeight)
        hbarT = fullHeight - hbarH;
    const hbar = this.container.selectAll('rect.scrollbar-horizontal').data((needsHorizontalScrollBar) ? [0] : []);
    hbar.exit()
        .on('.drag', null)
        .remove();
    ...
    let vbarL = l + w;
    const vbarT = t;
    if (vbarL + vbarW > fullWidth)
        vbarL = fullWidth - vbarW;
    const vbar = this.container.selectAll('rect.scrollbar-vertical').data((needsVerticalScrollBar) ? [0] : []);
    vbar.exit()
        .on('.drag', null)
        .remove();
 ...
 * @method
 */
ScrollBox.prototype._onBoxDrag = function _onBoxDrag(event) {
 ...
 * @method
 */
ScrollBox.prototype._onBoxWheel = function _onBoxWheel(event) {
    let translateX = this.translateX;
    let translateY = this.translateY;
    if (this.hbar) {
 ...
 * @method
 */
ScrollBox.prototype._onBarDrag = function _onBarDrag(event) {
    let translateX = this.translateX;
    let translateY = this.translateY;
    if (this.hbar) {
...
ScrollBox.prototype.setTranslate = function setTranslate(translateX, translateY) {
    ...
    this.container.call(setTranslate, this._box.l - this.position.l - translateX, this._box.t - this.position.t - translateY);
    if (this._clipRect) {
        this._clipRect
    ...
    if (this.hbar) {
        ...
        this.hbar.call(setTranslate, translateX + xf * this._hbarTranslateMax, translateY);
    }
    if (this.vbar) {
        const yf = translateY / translateYMax;
        this.vbar.call(setTranslate, translateX, translateY + yf * this._vbarTranslateMax);
    }
};
