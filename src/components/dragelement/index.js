...
dragElement.init = function init(options) {
    ...
    let startX, startY, newMouseDownTime, cursor, dragCover, initialEvent, initialTarget, rightClick;
    if (!gd._mouseDownTime)
        gd._mouseDownTime = 0;
    ...
    function onStart(e) {
        ...
        if (options.prepFn)
            options.prepFn(e, startX, startY);
    ...
    function onDone(e) {
        ...
        if (gd._dragged) {
            ...
            if (options.clickFn)
                options.clickFn(numClicks, clickEvent);
