...
color.combine = function (front, back) {
    ...
    if (fc.a === 1)
        return tinycolor(front).toRgbString();
...
color.contrast = function (cstr, lightAmount, darkAmount) {
    ...
    if (tc.getAlpha() !== 1)
        tc = tinycolor(color.combine(cstr, background));
...
color.clean = function (container) {
    ...
    for (i = 0; i < keys.length; i++) {
        ...
        else if (val && typeof val === 'object' && !isTypedArray(val))
            color.clean(val);
    }
};
