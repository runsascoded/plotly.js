...
function draw(gd, titleClass, options) {
    ...
    if (prop === 'title.text')
        editAttr = 'titleText';
    else if (prop.indexOf('axis') !== -1)
        editAttr = 'axisTitleText';
    else if (prop.indexOf('colorbar') !== -1)
        editAttr = 'colorbarTitleText';
    ...
    function scootTitle(titleAndSubtitleEls) {
        ...
        if (avoid && avoid.selection && avoid.side && txt) {
            ...
            const backside = OPPOSITE_SIDE[avoid.side];
    ...
    if (subtitleEl && !subtitleEl.empty())
        subtitleEl.classed('js-placeholder', subtitleIsPlaceholder);
    return group;
}
export default {
    draw: draw,
