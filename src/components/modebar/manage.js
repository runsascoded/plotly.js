...
export default function manageModeBar(gd) {
    ...
    if (modeBar)
        modeBar.update(gd, buttonGroups);
    else
        fullLayout._modeBar = createModeBar(gd, buttonGroups);
}
...
function getButtonGroups(gd) {
    ...
    function match(name, B) {
        if (typeof B === 'string') {
            if (B.toLowerCase() === name.toLowerCase())
                return true;
        }
        else {
            const v0 = B.name;
            const v1 = (B._cat || B.name);
            if (v0 === name || v1 === name.toLowerCase())
                return true;
        }
        return false;
    }
    let layoutAdd = fullLayout.modebar.add;
    if (typeof layoutAdd === 'string')
        layoutAdd = [layoutAdd];
    let layoutRemove = fullLayout.modebar.remove;
    if (typeof layoutRemove === 'string')
        layoutRemove = [layoutRemove];
    let buttonsToAdd = context.modeBarButtonsToAdd.concat(layoutAdd.filter((e) => {
        for (let i = 0; i < context.modeBarButtonsToRemove.length; i++) {
            if (match(e, context.modeBarButtonsToRemove[i]))
                return false;
        }
        return true;
    }));
    const buttonsToRemove = context.modeBarButtonsToRemove.concat(layoutRemove.filter((e) => {
        for (let i = 0; i < context.modeBarButtonsToAdd.length; i++) {
            if (match(e, context.modeBarButtonsToAdd[i]))
                return false;
        }
        return true;
    }));
    ...
    if (context.showEditInChartStudio)
        commonGroup.push('editInChartStudio');
    else if (context.showSendToCloud)
        commonGroup.push('sendDataToCloud');
    ...
    if (hasCartesian && !allAxesFixed) {
        ...
        if (resetGroup[0] !== 'resetViews')
            resetGroup = ['resetScale2d'];
    }
    if (hasGL3D) {
    ...
    const enableHover = (a) => {
        ...
        if (enabledHoverGroup.indexOf(a) !== -1)
            return;
...
function isSelectable(fullData) {
    ...
    for (let i = 0; i < fullData.length; i++) {
        ...
        if (!trace._module || !trace._module.selectPoints)
            continue;
