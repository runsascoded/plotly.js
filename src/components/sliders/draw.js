...
function drawCurrentValue(sliderGroup, sliderOpts, valueOverride) {
    ...
    const text = Lib.ensureSingle(sliderGroup, 'text', constants.labelClass, function (s) {
        s
    ...
    if (typeof valueOverride === 'string') {
        ...
        if (_meta)
            curVal = Lib.templateString(curVal, _meta);
        str += curVal;
    }
...
function drawGrip(sliderGroup, gd, sliderOpts) {
    const grip = Lib.ensureSingle(sliderGroup, 'rect', constants.gripRectClass, function (s) {
...
function drawLabel(item, data, sliderOpts) {
    const text = Lib.ensureSingle(item, 'text', constants.labelClass, function (s) {
        s
...
function drawLabelGroup(sliderGroup, sliderOpts) {
    ...
    labelItemsJoin.merge(labelItemsEnter).each(function (d) {
        const item = select(this);
        item.call(drawLabel, d, sliderOpts);
        setTranslate(item, normalizedValueToPosition(sliderOpts, d.fraction), constants.tickOffset +
            sliderOpts.ticklen +
...
function drawTicks(sliderGroup, sliderOpts) {
    ...
    tickMerged.each(function (d, i) {
        ...
        setTranslate(item, normalizedValueToPosition(sliderOpts, i / (sliderOpts._stepCount - 1)) - 0.5 * sliderOpts.tickwidth, (isMajor ? constants.tickOffset : constants.minorTickOffset) + dims.currentValueTotalHeight);
    });
}
...
function setGripPosition(sliderGroup, sliderOpts, doTransition) {
    ...
    if (sliderOpts._invokingCommand)
        return;
    let el = grip;
...
function drawTouchRect(sliderGroup, gd, sliderOpts) {
    const dims = sliderOpts._dims;
    const rect = Lib.ensureSingle(sliderGroup, 'rect', constants.railTouchRectClass, function (s) {
        s.call(attachGripEvents, gd, sliderGroup, sliderOpts)
            .style('pointer-events', 'all');
    });
    rect
...
function drawRail(sliderGroup, sliderOpts) {
    ...
    rect
        ...
        .call(Color.fill, sliderOpts.bgcolor)
        .style('stroke-width', sliderOpts.borderwidth + 'px');
    setTranslate(rect, constants.railInset, (dims.inputAreaWidth - constants.railWidth) * 0.5 + dims.currentValueTotalHeight);
}
