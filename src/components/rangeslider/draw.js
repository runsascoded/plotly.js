...
export default function (gd) {
    ...
    if (rangeSliderData.length === 0)
        return;
    const rangeSliderEnter = rangeSliders.enter().append('g')
        .classed(constants.containerClassName, true)
        .attr('pointer-events', 'all');
    // for all present range sliders
    rangeSliders.merge(rangeSliderEnter).each(function (axisOpts) {
        ...
        const y = Math.round(gs.t + gs.h * (1 - axisOpts._counterDomainMin) +
...
function drawBg(rangeSlider, gd, axisOpts, opts) {
    const bg = Lib.ensureSingle(rangeSlider, 'rect', constants.bgClassName, function (s) {
        s
            .attr('x', 0)
            .attr('y', 0)
...
function addClipPath(rangeSlider, gd, axisOpts, opts) {
    const fullLayout = gd._fullLayout;
    const clipPath = Lib.ensureSingleById(fullLayout._topdefs, 'clipPath', opts._clipId, function (s) {
        s.append('rect')
            .attr('x', 0)
            .attr('y', 0);
    });
...
function drawRangePlot(rangeSlider, gd, axisOpts, opts) {
    ...
    let mainplotinfo;
    rangePlotsMerged.each(function (id, i) {
...
function drawMasks(rangeSlider, gd, axisOpts, opts, oppAxisRangeOpts) {
    const maskMin = Lib.ensureSingle(rangeSlider, 'rect', constants.maskMinClassName, function (s) {
        s
            .attr('x', 0)
            .attr('y', 0)
            .attr('shape-rendering', 'crispEdges');
    ...
    const maskMax = Lib.ensureSingle(rangeSlider, 'rect', constants.maskMaxClassName, function (s) {
        s
            .attr('y', 0)
            .attr('shape-rendering', 'crispEdges');
    });
    ...
    if (oppAxisRangeOpts.rangemode !== 'match') {
        const maskMinOppAxis = Lib.ensureSingle(rangeSlider, 'rect', constants.maskMinOppAxisClassName, function (s) {
            s
                .attr('y', 0)
                .attr('shape-rendering', 'crispEdges');
        });
        ...
        const maskMaxOppAxis = Lib.ensureSingle(rangeSlider, 'rect', constants.maskMaxOppAxisClassName, function (s) {
            s
                .attr('y', 0)
                .attr('shape-rendering', 'crispEdges');
        });
...
function drawSlideBox(rangeSlider, gd, axisOpts, opts) {
    if (gd._context.staticPlot)
        return;
    const slideBox = Lib.ensureSingle(rangeSlider, 'rect', constants.slideBoxClassName, function (s) {
        s
            .attr('y', 0)
...
function drawGrabbers(rangeSlider, gd, axisOpts, opts) {
    ...
    // <g grabarea />
    function setGrabAreaFixAttrs(s) {
