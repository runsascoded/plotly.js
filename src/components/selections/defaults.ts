...
export default function supplyLayoutDefaults(layoutIn: any, layoutOut: FullLayout) {
    ...
    const selections = layoutOut.selections;
...
function handleSelectionDefaults(selectionIn: any, selectionOut: any, fullLayout: FullLayout) {
    ...
    const selectionType = coerce('type', dfltType);
    const noPath = selectionType !== 'path';
    ...
    for(let i = 0; i < 2; i++) {
        ...
        // xref, yref
        const axRef = Axes.coerceRef(selectionIn, selectionOut, gdMock, axLetter);
        ...
        if(noPath) {
            ...
            const in0 = selectionIn[attr0];
            const in1 = selectionIn[attr1];
            ...
            const p0 = selectionOut[attr0];
            const p1 = selectionOut[attr1];
