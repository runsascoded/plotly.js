...
function draw(gd) {
    ...
    for (const k in fullLayout._plots) {
        ...
        if (selectionLayer)
            selectionLayer.selectAll('path').remove();
    }
    ...
    function drawSelection(selectionLayer) {
        ...
        for (let sensory = 1; sensory >= 0; sensory--) {
            const path = selectionLayer.append('path')
                ...
                .call(dashLine, sensory ? 'solid' : lineDash, sensory ? 4 + lineWidth : lineWidth);
...
function setClipPath(selectionPath, gd, selectionOptions) {
    ...
    setClipUrl(selectionPath, 'clip' + gd._fullLayout._uid + clipAxes, gd);
}
function activateSelection(gd, path) {
    if (!couldHaveActiveSelection(gd))
        return;
