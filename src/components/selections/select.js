...
function prepSelect(evt, startX, startY, dragOptions, mode) {
    ...
    const strokeC = newStyle.line.color || ((isCartesian ? Color.contrast(gd._fullLayout.plot_bgcolor) : '#7f7f7f') // non-cartesian subplot
    );
    outlines.enter()
        .append('path')
    ...
    const searchTraces = determineSearchTraces(gd, dragOptions.xaxes, dragOptions.yaxes, dragOptions.subplot);
            ...
            throttle.throttle(throttleID, constants.SELECTDELAY, function () {
                ...
                for (let w = 0; w < extraPoints.length; w++) {
                    ...
                    for (let u = 0; u < newPoints.length; u++) {
                        if (newPoints[u].curveNumber === p.curveNumber &&
                ...
                if (searchTraces.length) {
                    ...
                    if (clickedXaxis && clickedYaxis) {
                        ...
                        for (let k = 0; k < allSelections.length; k++) {
                            ...
                            if (!s)
                                continue; // also drop null selections if any
                            if (s.xref !== clickedXaxis._id ||
 ...
 */
function multiTester(list, _selectionTesters) {
    if (!list.length)
        return;
    const testers = [];
...
function coerceSelectionsCache(evt, gd, dragOptions) {
    ...
    const selectingOnSameSubplot = (fullLayout._lastSelectedSubplot &&
...
function determineSearchTraces(gd, xAxes, yAxes, subplot) {
    ...
    for (i = 0; i < gd.calcdata.length; i++) {
        cd = gd.calcdata[i];
        trace = cd[0].trace;
        if (trace.visible !== true || !trace._module || !trace._module.selectPoints)
            continue;
        if (hasSubplot({ subplot: subplot }) &&
            ...
            if (xAxisIds.indexOf(trace.xaxis) === -1 && (!trace._xA || !trace._xA.overlaying))
                continue;
            if (yAxisIds.indexOf(trace.yaxis) === -1 && (!trace._yA || !trace._yA.overlaying))
                continue;
...
function updateReglSelectedState(gd, searchTraces) {
    ...
    for (let i = 0; i < searchTraces.length; i++) {
        ...
        if (fn) {
            fn(gd, cd, cd[0].node3);
            if (cd[0].nodeRangePlot3)
                fn(gd, cd, cd[0].nodeRangePlot3);
        }
    }
    if (hasRegl) {
        clearGlCanvases(gd);
...
function reselect(gd, mayEmitSelected, selectionTesters, searchTraces, dragOptions) {
    ...
    const eventData = { points: allSelections };
...
function addTester(layoutPolygons, xRef, yRef, selectionTesters) {
    ...
    for (let i = 0; i < layoutPolygons.length; i++) {
        ...
        if (xRef !== currentPolygon.xref || yRef !== currentPolygon.yref)
            continue;
        if (mergedPolygons) {
...
function getLayoutPolygons(gd, onlyActiveOnes) {
    ...
    for (let i = 0; i < len; i++) {
        if (onlyActiveOnes && i !== fullLayout._activeSelectionIndex)
            continue;
