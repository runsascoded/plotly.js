...
function drawRaw(gd, options, index, subplotId, xa, ya) {
    ...
    function drawGraphicalElements() {
        ...
        const anntextBB = bBox((hasMathjax ? mathjaxGroup : annText).node());
        ...
        annTextClip.select('rect').call(setRect, borderfull, borderfull, annWidth, annHeight);
        annTextBG.call(setRect, borderwidth / 2, borderwidth / 2, outerWidth - borderwidth, outerHeight - borderwidth);
        annTextGroupInner.call(setTranslate, Math.round(annPosPx.x.text - outerWidth / 2), Math.round(annPosPx.y.text - outerHeight / 2));
        /*
        ...
        const drawArrow = (dx, dy) => {
            ...
            if (edges.reduce((a, x) => {
                return a ^
                    !!Lib.segmentsIntersect(headX, headY, headX + 1e6, headY + 1e6, x[0], x[1], x[2], x[3]);
            }, false)) {
                // no line or arrow - so quit drawArrow now
                return;
            }
            edges.forEach((x) => {
                const p = Lib.segmentsIntersect(tailX, tailY, headX, headY, x[0], x[1], x[2], x[3]);
                if (p) {
                    tailX = p.x;
                    tailY = p.y;
                }
            ...
            if (edits.annotationPosition && arrow.node().parentNode && !subplotId) {
                ...
                dragElement.init({
                    ...
                    doneFn: function () {
                        ...
                        if (notesBox)
                            notesBox.redraw(notesBox.selectedObj);
                    }
                });
            }
        };
