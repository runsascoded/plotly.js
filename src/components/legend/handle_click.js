...
export default function handleClick(g, gd, numClicks) {
    ...
    if (legendItem.groupTitle && legendItem.noClick)
        return;
    const fullData = gd._fullData;
    const shapesWithLegend = (fullLayout.shapes || []).filter((d) => d.showlegend);
    ...
    const updatedShapes = (fullLayout.shapes || []).map((d) => d._input);
    ...
    if (!isShape && traceIs(fullTrace, 'pie-like')) {
        ...
        if (mode === 'toggle') {
            if (thisLabelIndex === -1)
                hiddenSlices.push(thisLabel);
            else
                hiddenSlices.splice(thisLabelIndex, 1);
        }
            ...
            for (i = 0; i < gd.calcdata.length; i++) {
                ...
                for (j = 0; j < cdi.length; j++) {
                    ...
                    if (thisLegend === cdi[0].trace.legend) {
                        if (thisLabel !== dLabel) {
                            if (hiddenSlices.indexOf(dLabel) === -1)
                                changed = true;
        ...
        if (mode === 'toggle') {
            ...
            for (i = 0; i < allLegendItems.length; i++) {
                ...
                if (_item.visible === false || _item.legend !== thisLegend)
                    continue;
