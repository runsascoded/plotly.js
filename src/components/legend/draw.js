...
function horizontalAlignTitle(titleEl, legendObj, bw) {
    if ((legendObj.title.side !== 'top center') && (legendObj.title.side !== 'top right'))
        return;
...
function drawOne(gd, opts) {
    ...
    traces.style('opacity', function (d) {
        ...
        .each(function () { if (!inHover)
        select(this).call(setupTraceToggle, gd, legendId); });
    syncOrAsync([
        previousPromises,
        ...
        function () {
            ...
            if (inHover || legendObj._height <= legendObj._maxHeight || gd._context.staticPlot) {
                ...
                const scrollBarHeight = Math.max(constants.scrollBarMinHeight, legendObj._effHeight * legendObj._effHeight / legendObj._height);
                ...
                legend.on('wheel', function (event) {
                    scrollBoxY = constrain(legendObj._scrollY +
                        ((event.deltaY / scrollBoxYMax) * scrollBarYMax), 0, scrollBoxYMax);
                    scrollHandler(scrollBoxY, scrollBarHeight, scrollRatio);
                ...
                const getScrollBarDragY = (scrollBoxY0, eventY0, eventY1) => {
                    const y = ((eventY1 - eventY0) / scrollRatio) + scrollBoxY0;
                    return constrain(y, 0, scrollBoxYMax);
                };
                const getNaturalDragY = (scrollBoxY0, eventY0, eventY1) => {
            ...
            function scrollHandler(scrollBoxY, scrollBarHeight, scrollRatio) {
                ...
                setRect(scrollBar, legendObj._width, constants.scrollBarMargin + scrollBoxY * scrollRatio, constants.scrollBarWidth, scrollBarHeight);
            ...
            if (gd._context.edits.legendPosition) {
                ...
                dragElement.init({
                    ...
                    clickFn: function (numClicks, e) {
                        const clickedTrace = layer.selectAll('g.traces').filter(function () {
...
function getTraceWidth(d, legendObj, textGap, isGrouped) {
...
function clickOrDoubleClick(gd, legend, legendItem, numClicks, evt) {
    ...
    if (numClicks === 1) {
        ...
        if (dblClickVal !== false && clickVal !== false)
            handleClick(legendItem, gd, numClicks);
    }
}
...
function ensureLength(str, maxLength) {
    ...
    if (str && str.trim().length >= targetLength / 2)
        return str;
    str = str || '';
...
function textLayout(s, g, gd, legendObj, aTitle) {
    if (legendObj._inHover)
        s.attr('data-notex', true); // do not process MathJax for unified hover
...
function computeTextDimensions(g, gd, legendObj, aTitle) {
    ...
    if (mathjaxNode) {
        ...
        const cls = '.' + legendId + (aTitle === MAIN_TITLE ? 'title' : '') + 'text';
...
function computeLegendDimensions(gd, groups, traces, legendObj) {
    ...
    if (isVertical) {
        traces.each(function (d) {
            const h = d[0].height;
            setTranslate(this, bw + titleSize[0], bw + titleSize[1] + legendObj._height + h / 2 + itemGap);
            legendObj._height += h;
        ...
        legendObj._maxWidth = Math.max(isLeftOfPlotArea ? ((isBeyondPlotAreaY && xanchor === 'left') ? gs.l + gs.w : hw) :
        ...
        if (isGrouped) {
            ...
            groups.each(function () {
                ...
                select(this).selectAll('g.traces').each(function (d) {
                    const w = getTraceWidth(d, legendObj, textGap);
                    const h = d[0].height;
                    setTranslate(this, titleSize[0], titleSize[1] + bw + itemGap + h / 2 + offsetY);
                    offsetY += h;
            ...
            traces.each(function (d) {
                ...
                setTranslate(this, titleSize[0] + bw + offsetX, titleSize[1] + bw + offsetY + h / 2 + itemGap);
