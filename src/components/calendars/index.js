...
const handleDefaults = (contIn, contOut, attr, dflt) => {
    const attrs = {};
    attrs[attr] = attributes;
    return Lib.coerce(contIn, contOut, attrs, attr, dflt);
};
const handleTraceDefaults = (traceIn, traceOut, coords, layout) => {
...
function worldCalFmt(fmt, x, calendar) {
    ...
    while ((i = fmt.indexOf('%', i)) !== -1) {
        ...
        if (!directiveObj) {
            ...
            if (directiveObj === UNKNOWN)
                replacementPart = UNKNOWN;
