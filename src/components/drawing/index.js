...
export function font(s, font) {
    ...
    if (color)
        s.call(Color.fill, color);
    if (weight)
        s.style('font-weight', weight);
    if (style)
        s.style('font-style', style);
    if (variant)
        s.style('font-variant', variant);
    if (textcase)
        s.style('text-transform', dropNone(textcase2transform(textcase)));
    if (shadow)
        s.style('text-shadow', shadow === 'auto' ? svgTextUtils.makeTextShadow(Color.contrast(color)) : dropNone(shadow));
    if (lineposition)
        s.style('text-decoration-line', dropNone(lineposition2decorationLine(lineposition)));
}
function dropNone(a) {
    return a === 'none' ? undefined : a;
}
const textcase2transformOptions = {
    normal: 'none',
    lower: 'lowercase',
    upper: 'uppercase',
...
export function hideOutsideRangePoints(traceGroups, subplot) {
    if (!subplot._hasClipOnAxisFalse)
        return;
...
export function crispRound(gd, lineWidth, dflt) {
    if (!lineWidth || !isNumeric(lineWidth))
        return dflt || 0;
...
export function dashStyle(dash, lineWidth) {
    ...
    if (dash === 'solid')
        ...
        dash = 3 * dlw + 'px,' + 3 * dlw + 'px';
    else if (dash === 'longdash')
        dash = 5 * dlw + 'px,' + 5 * dlw + 'px';
...
function setFillStyle(sel, trace, gd, forLegend) {
    ...
    if (patternShape) {
        ...
        pattern(sel, 'point', gd, patternID, patternShape, patternSize, patternSolidity, undefined, markerPattern.fillmode, patternBGColor, patternFGColor, patternFGOpacity);
    }
...
export const symbolFuncs = [];
export const symbolBackOffs = [];
export const symbolNeedLines = {};
export const symbolNoDot = {};
export const symbolNoFill = {};
export const symbolList = [];
Object.keys(SYMBOLDEFS).forEach((k) => {
    const symDef = SYMBOLDEFS[k];
    const n = symDef.n;
    symbolList.push(n, String(n), k, n + 100, String(n + 100), k + '-open');
    symbolNames[n] = k;
    ...
    if (symDef.noDot) {
        ...
        symbolList.push(n + 200, String(n + 200), k + '-dot', n + 300, String(n + 300), k + '-open-dot');
    }
    if (symDef.noFill) {
        symbolNoFill[n] = true;
    }
...
function gradientWithBounds(sel, gd, gradientID, type, colorscale, prop, start, stop, inUserSpace, reversed) {
    ...
    gradient
        ...
        if (info.attrs) {
            for (const k in info.attrs)
                el.attr(k, info.attrs[k]);
        }
        el.attr('id', fullID);
        const stops = el.selectAll('stop').data(colorStops);
        stops.exit().remove();
        const stopsEnter = stops.enter().append('stop');
        stops.merge(stopsEnter).each(function (d) {
...
export function pattern(sel, calledBy, gd, patternID, shape, size, solidity, mcc, fillmode, bgcolor, fgcolor, fgopacity) {
    ...
    let width, height;
    const linearFn = function (x, x0, x1, y0, y1) {
        return y0 + ((y1 - y0) * (x - x0)) / (x1 - x0);
    };
    let path, linewidth, radius;
    let patternTag;
    let patternAttrs = {};
    ...
    pattern
        ...
        for (const k in patternAttrs)
            patternEl.attr(k, patternAttrs[k]);
    });
...
export function singlePointStyle(d, sel, trace, fns, gd, pt) {
    ...
    let fillColor, lineColor, lineWidth;
    if (d.so) {
        ...
        if ('mlc' in d)
            lineColor = d.mlcc = fns.lineScale(d.mlc);
        else if (isArrayOrTypedArray(markerLine.color))
            lineColor = Color.defaultLine;
        else
            lineColor = markerLine.color;
    ...
    if (d.om) {
        ...
        if (gradientType)
            ...
            gradientType = markerGradient && markerGradient.type;
        if (isArrayOrTypedArray(gradientType)) {
            gradientType = gradientType[0];
            if (!gradientInfo[gradientType])
                gradientType = 0;
        }
        ...
        if (gradientType && gradientType !== 'none') {
            ...
            if (gradientColor)
                ...
                gradientColor = markerGradient.color;
            ...
            if (perPointPattern)
                patternID += '-' + d.i;
            pattern(sel, 'point', gd, patternID, patternShape, patternSize, patternSolidity, d.mcc, markerPattern.fillmode, patternBGColor, patternFGColor, patternFGOpacity);
        }
        else {
    ...
    out.selectedTextColorFn = function (d) {
        ...
        if (d.selected) {
            ...
            if (utc)
                ...
                return stc ? base : Color.addOpacity(base, DESELECTDIM);
        }
    };
    return out;
}
export function selectedPointStyle(s, trace) {
    if (!s.size() || !trace.selectedpoints)
        return;
    ...
    if (fns.selectedSizeFn) {
        seq.push(function (pt, d) {
            ...
            pt.attr('d', makePointPath(symbolNumber(mx), mrc2, getMarkerAngle(d, trace), getMarkerStandoff(d, trace)));
            d.mrc2 = mrc2;
        });
    }
    if (seq.length) {
...
const STEPLINEAR = function (p0, p1, isLastPoint) {
...
function applyBackoff(pt, start) {
    ...
    if (backoff &&
        ...
        if (b === 'auto') {
            ...
            if (isArrayOrTypedArray(endMarkerSymbol))
                endMarkerSymbol = endMarkerSymbol[endI];
            let endMarkerSize = endMarker.size;
            if (isArrayOrTypedArray(endMarkerSize))
                endMarkerSize = endMarkerSize[endI];
...
export function makeTester() {
    const _tester = ensureSingleById(select('body'), 'svg', 'js-plotly-tester', function (s) {
    ...
    const _testref = ensureSingle(_tester, 'path', 'js-reference-point', function (s) {
...
export function bBox(node, inTester, hash) {
    ...
    if (!inTester)
        testerNode.removeChild(testNode);
    const bb = {
