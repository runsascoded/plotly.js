...
const iQS = [0, 3, 4, 1, 2];
export function writePaths(polygons) {
...
export function readPaths(str, gd, plotinfo, isActiveShape) {
...
export function pointsOnRectangle(cell) {
...
export function pointsOnEllipse(cell) {
    let len = cell.length;
    if (len !== CIRCLE_SIDES + 1)
        return false;
    ...
    for (let i = 0; i < len; i++) {
        ...
        if (!almostEq(dist(cell[i], cell[i2]), dist(cell[k], cell[k2])))
            return false;
    }
    return true;
}
export function handleEllipse(isEllipse, start, end) {
    if (!isEllipse)
        return [start, end]; // i.e. case of line
...
export function ellipseOver(pos) {
    let x0 = pos.x0;
    let y0 = pos.y0;
    const x1 = pos.x1;
    const y1 = pos.y1;
...
export function fixDatesForPaths(polygons, xaxis, yaxis) {
    ...
    for (let i = 0; i < polygons.length; i++) {
        for (let j = 0; j < polygons[i].length; j++) {
            for (let k = 0; k + 2 < polygons[i][j].length; k += 2) {
                if (xIsDate)
                    polygons[i][j][k + 1] = polygons[i][j][k + 1].replace(' ', '_');
                if (yIsDate)
                    polygons[i][j][k + 2] = polygons[i][j][k + 2].replace(' ', '_');
            }
        }
    }
    return polygons;
