...
function drawOne(gd, index) {
    ...
    function drawShape(shapeLayer) {
        ...
        let editHelpers;
        if (isActiveShape || gd._context.edits.shapePosition)
            editHelpers = arrayEditor(gd.layout, 'shapes', options);
        if (isActiveShape) {
...
function setClipPath(shapePath, gd, shapeOptions) {
    ...
    setClipUrl(shapePath, (clipAxes ? 'clip' + gd._fullLayout._uid + clipAxes : null), gd);
}
function setupDragElement(gd, shapePath, shapeOptions, index, shapeLayer, editHelpers) {
    ...
    function moveShape(dx, dy) {
        if (shapeOptions.type === 'path') {
            ...
            if (xPixelSized) {
                ...
                if (xa && xa.type === 'date')
                    moveX = helpers.encodeDate(moveX);
            }
            if (yPixelSized) {
                ...
                if (ya && ya.type === 'date')
                    moveY = helpers.encodeDate(moveY);
            }
    ...
    function resizeShape(dx, dy) {
        if (isPath) {
            ...
            const has = (str) => { return dragMode.indexOf(str) !== -1; };
            const hasN = has('n');
            const hasS = has('s');
            const hasW = has('w');
            const hasE = has('e');
    ...
    function deactivateClipPathTemporarily(shapePath, shapeOptions, gd) {
        ...
        let clipAxes = '';
        if (xref !== 'paper' && !xa.autorange)
            clipAxes += xref;
        if (yref !== 'paper' && !ya.autorange)
            clipAxes += yref;
        setClipUrl(shapePath, (clipAxes ? 'clip' + gd._fullLayout._uid + clipAxes : null), gd);
    }
}
function movePath(pathIn, moveX, moveY) {
    return pathIn.replace(constants.segmentRE, function (segment) {
        ...
        const paramString = segment.slice(1).replace(constants.paramRE, function (param) {
