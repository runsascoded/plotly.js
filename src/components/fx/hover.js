...
const multipleHoverPoints = {
    box: true,
    ohlc: true,
    violin: true,
    candlestick: true
};
const cartesianScatterPoints = {
    scatter: true,
    scattergl: true,
    splom: true
};
...
export function hover(gd, evt, subplot, noHoverEvent) {
    gd = getGraphDiv(gd);
...
export function loneHover(hoverItems, opts) {
    let multiHover = true;
...
function _hover(gd, evt, subplot, noHoverEvent, eventTarget) {
    ...
    if (plotinfo && hoversubplots !== 'single') {
        const overlayedSubplots = plotinfo.overlays.map((pi) => pi.id);
    ...
    if (hovermode && !supportsCompare)
        hovermode = 'closest';
    if (['x', 'y', 'closest', 'x unified', 'y unified'].indexOf(hovermode) === -1 ||
        !gd.calcdata ||
    ...
    if (Array.isArray(evt)) {
        ...
        if (hasUserCalledHover) {
            if ('xpx' in evt)
                ...
                xpx = xaArray[0]._length / 2;
            if ('ypx' in evt)
                ypx = evt.ypx;
            else
                ypx = yaArray[0]._length / 2;
        }
        else {
        ...
        if ('xval' in evt)
            xvalArray = helpers.flat(subplots, evt.xval);
        else
            xvalArray = helpers.p2c(xaArray, xpx);
        if ('yval' in evt)
            yvalArray = helpers.flat(subplots, evt.yval);
        else
    ...
    function selectClosestPoint(pointsData, spikedistance, spikeOnWinning) {
        ...
        for (let i = 0; i < pointsData.length; i++) {
            if (firstXaxis && firstXaxis._id !== pointsData[i].xa._id)
                continue;
            if (firstYaxis && firstYaxis._id !== pointsData[i].ya._id)
                continue;
            thisSpikeDistance = pointsData[i].spikeDistance;
            if (spikeOnWinning && i === 0)
                thisSpikeDistance = -Infinity;
    ...
    const rotateLabels = (hovermode === 'y' && (searchData.length > 1 || hoverData.length > 1)) ||
    ...
    if (!eventTarget || noHoverEvent || !hoverChanged(gd, evt, oldhoverdata))
        return;
    if (oldhoverdata) {
...
function createHoverText(hoverData, opts) {
    ...
    let showCommonLabel = t0 !== undefined && c0.distance <= opts.hoverdistance && (hovermode === 'x' || hovermode === 'y');
    ...
    if (showCommonLabel) {
        ...
        for (i = 0; i < hoverData.length; i++) {
            if (allHaveZ && hoverData[i].zLabel === undefined)
                allHaveZ = false;
    ...
    if (helpers.isUnifiedHover(hovermode)) {
        ...
        if (axLetter === 'y') {
            if (pointWon) {
                ...
                lyTop = Math.min.apply(null, groupedHoverData.map((c) => Math.min(c.y0, c.y1)));
                lyBottom = Math.max.apply(null, groupedHoverData.map((c) => Math.max(c.y0, c.y1)));
            }
        }
        else {
            lyTop = lyBottom =
                mean(groupedHoverData.map((c) => (c.y0 + c.y1) / 2)) -
                    tHeight / 2;
        }
        let lxRight, lxLeft;
        if (axLetter === 'x') {
            if (pointWon) {
                ...
                lxRight = Math.max.apply(null, groupedHoverData.map((c) => Math.max(c.x0, c.x1)));
                lxLeft = Math.min.apply(null, groupedHoverData.map((c) => Math.min(c.x0, c.x1)));
            }
        }
        else {
            lxRight = lxLeft =
                mean(groupedHoverData.map((c) => (c.x0 + c.x1) / 2)) -
                    tWidth / 2;
        }
    ...
    const hoverLabelsJoin = container.selectAll('g.hovertext').data(hoverData, function (d) {
...
function getHoverLabelText(d, showCommonLabel, hovermode, fullLayout, t0, g) {
    ...
    if (d.nameOverride !== undefined)
        d.name = d.nameOverride;
    if (d.name) {
        if (d.trace._meta)
            d.name = templateString(d.name, d.trace._meta);
...
function alignHoverText(hoverLabels, rotateLabels, scaleX, scaleY) {
    ...
    hoverLabels.each(function (d) {
        ...
        if (d.tx2width) {
            ...
            g.select('rect').call(setRect, pX(shiftX.text2ShiftX + ((shiftX.alignShift - 1) * d.tx2width) / 2 + offsetX), pY(offsetY - d.by / 2 - 1), pX(d.tx2width), pY(d.by + 2));
        }
    });
}
function cleanPoint(d, hovermode) {
    ...
    if (infomode && infomode !== 'all') {
        ...
        if (infomode.indexOf('x') === -1)
            d.xLabel = undefined;
        if (infomode.indexOf('y') === -1)
            d.yLabel = undefined;
        if (infomode.indexOf('z') === -1)
            d.zLabel = undefined;
        if (infomode.indexOf('text') === -1)
            d.text = undefined;
        if (infomode.indexOf('name') === -1)
            d.name = undefined;
    }
    return d;
}
function createSpikelines(gd, closestPoints, opts) {
    ...
    if (showY) {
        ...
        const dfltHLineColor = tinycolor.readability(hLinePoint.color, contrastColor) < 1.5
    ...
    if (showX) {
        ...
        const dfltVLineColor = tinycolor.readability(vLinePoint.color, contrastColor) < 1.5
            ? Color.contrast(contrastColor)
            : vLinePoint.color;
