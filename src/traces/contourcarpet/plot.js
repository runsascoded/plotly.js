...
export default function plot(gd, plotinfo, cdcontours, contourcarpetLayer) {
    ...
    Lib.makeTraceGroups(contourcarpetLayer, cdcontours, 'contour').each(function (cd) {
        ...
        if (!carpet.visible || carpet.visible === 'legendonly')
            return;
        const a = cd0.a;
        const b = cd0.b;
...
function mapPathinfo(pathinfo, map) {
    let i, j, k, pi, pedgepaths, ppaths, pedgepath, ppath, path;
        ...
        linegroup.each(function (d) {
            ...
            select(this).selectAll('path').each(function (pathData) {
                ...
                if (pathBounds.len < (textOpts.width + textOpts.height) * constants.LABELMIN)
                    return;
                const maxLabels = Math.min(Math.ceil(pathBounds.len / normLength), constants.LABELMAX);
                for (let i = 0; i < maxLabels; i++) {
                    const loc = contourPlot.findBestTextLocation(path, pathBounds, textOpts, labelData, bounds);
                    if (!loc)
                        break;
...
function constrainToCarpet(path, pathData, levelData, pathBounds, carpet, textHeight) {
    ...
    if (pathBounds.min < minStart)
        pathBounds.min = minStart;
    if (pathBounds.max > maxEnd)
        pathBounds.max = maxEnd;
...
function makeFills(trace, plotgroup, xa, ya, pathinfo, perimeter, ab2p, carpet, carpetcd, coloring, boundaryPath) {
    ...
    fillitems.merge(fillitemsEnter).each(function (pi) {
...
function joinAllPaths(trace, pi, perimeter, ab2p, carpet, carpetcd, xa, ya) {
    let i;
    let fullpath = '';
    const startsleft = pi.edgepaths.map((v, i) => i);
    let newloop = true;
