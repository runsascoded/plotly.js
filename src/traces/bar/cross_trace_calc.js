        ...
        if (fullTrace.visible === true &&
            ...
            if (fullTrace._computePh) {
                ...
                for (let j = 0; j < cd.length; j++) {
                    if (typeof cd[j].ph0 === 'function')
                        cd[j].ph0 = cd[j].ph0();
                    if (typeof cd[j].ph1 === 'function')
                        cd[j].ph1 = cd[j].ph1();
                }
            }
        }
    ...
    switch (opts.mode) {
        ...
        case 'group':
            ...
            for (i = 0; i < calcTraces.length; i++) {
                ...
                if (fullTrace.offset === undefined)
                    included.push(calcTrace);
                else
                    excluded.push(calcTrace);
            }
            if (included.length) {
    ...
    for (i = 0; i < calcTraces.length; i++) {
        ...
        if (isArrayOrTypedArray(width)) {
            ...
            for (j = 0; j < newBarwidth.length; j++) {
                if (!isNumeric(newBarwidth[j]))
                    newBarwidth[j] = initialBarwidth;
            }
            // if the length of the array is too short,
...
function setBaseAndTop(sa, sieve) {
    ...
    for (let i = 0; i < calcTraces.length; i++) {
        ...
        for (let j = 0; j < calcTrace.length; j++) {
            ...
            const top = isScatter ? (isVertical ? bar.y : bar.x) : base + bar.s;
            bar[sLetter] = top;
            pts.push(top);
            if (bar.hasB)
                pts.push(base);
