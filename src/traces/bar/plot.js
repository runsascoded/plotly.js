...
function plot(gd, plotinfo, cdModule, traceLayer, opts, makeOnCompleteCallback) {
    ...
    const bartraces = makeTraceGroups(traceLayer, cdModule, 'trace bars').each(function (cd) {
        ...
        bars.merge(barsEnter).each(function (di, i) {
            ...
            function roundWithLine(v, _vc, _hideZeroSpan) {
...
function scaleTextForRoundedBar(x0, x1, y0, y1, t, r, overhead, isHorizontal, hasB) {
    ...
    if (isHorizontal) {
        pad = Math.max(0, R -
            Math.sqrt(Math.max(0, R * R - (R - (barHeight - t.y * scale) / 2) * (R - (barHeight - t.y * scale) / 2))) -
            overhead);
    }
    else {
        pad = Math.max(0, R -
            Math.sqrt(Math.max(0, R * R - (R - (barWidth - t.x * scale) / 2) * (R - (barWidth - t.x * scale) / 2))) -
            overhead);
    }
...
function calcTexttemplate(fullLayout, cd, index, xa, ya) {
    ...
    if (isHistogram || pt.x === undefined)
        pt.x = isHorizontal ? obj.value : obj.label;
    if (isHistogram || pt.y === undefined)
        pt.y = isHorizontal ? obj.label : obj.value;
    if (isHistogram || pt.xLabel === undefined)
        pt.xLabel = isHorizontal ? obj.valueLabel : obj.labelLabel;
    if (isHistogram || pt.yLabel === undefined)
        pt.yLabel = isHorizontal ? obj.labelLabel : obj.valueLabel;
    if (isWaterfall) {
