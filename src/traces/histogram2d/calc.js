...
export default function calc(gd, trace) {
    ...
    const xr2c = (v) => { return xa.r2c(v, 0, xcalendar); };
    const yr2c = (v) => { return ya.r2c(v, 0, ycalendar); };
    const xc2r = (v) => { return xa.c2r(v, 0, xcalendar); };
    const yc2r = (v) => { return ya.c2r(v, 0, ycalendar); };
    let i, j, n, m;
    // calculate the bins
    ...
    if (!nonuniformBinsX && xa.type === 'date')
        xbins = binsToCalc(xr2c, xbins);
    if (!nonuniformBinsY && ya.type === 'date')
        ybins = binsToCalc(yr2c, ybins);
    // put data into bins
    ...
    for (i = 0; i < serieslen; i++) {
        ...
        if (n >= 0 && n < nx && m >= 0 && m < ny) {
            ...
            if (uniqueValsPerX) {
                if (xVals[n] === undefined)
                    xVals[n] = xi;
                else if (xVals[n] !== xi)
                    uniqueValsPerX = false;
            }
            if (uniqueValsPerY) {
                if (yVals[m] === undefined)
                    yVals[m] = yi;
                else if (yVals[m] !== yi)
                    uniqueValsPerY = false;
            }
