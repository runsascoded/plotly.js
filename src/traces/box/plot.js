...
function plotBoxAndWhiskers(sel, axes, trace, t, isStatic) {
    ...
    const paths = sel.selectAll('path.box').data((trace.type !== 'violin' ||
        trace.box.visible) ? Lib.identity : []);
    const pathsEnter = paths.enter().append('path')
        .style('vector-effect', isStatic ? 'none' : 'non-scaling-stroke')
        .attr('class', 'box');
    paths.exit().remove();
    paths.merge(pathsEnter).each(function (d) {
        if (d.empty)
            return select(this).attr('d', 'M0,0Z');
        ...
        const m = Lib.constrain(sdmode ? valAxis.c2p(d.mean, true) :
            valAxis.c2p(d.med, true), Math.min(q1, q3) + 1, Math.max(q1, q3) - 1);
        ...
        if (isHorizontal) {
            select(this).attr('d', 'M' + m + ',' + posm0 + 'V' + posm1 + // median line
            ...
            select(this).attr('d', 'M' + posm0 + ',' + m + 'H' + posm1 + // median line
        ...
        if (trace.jitter) {
            if (typicalSpread === 0) {
                ...
                for (i = 0; i < pts.length; i++) {
                    ...
                    if (mode !== 'all') {
                        if (pts[i].v < d.lf)
                            pmax = Math.min(pmax, d.lf);
                        else
                            pmin = Math.max(pmin, d.uf);
                    }
...
function plotBoxMean(sel, axes, trace, t) {
    ...
    const paths = sel.selectAll('path.mean').data(((trace.type === 'box' && trace.boxmean) ||
        (trace.type === 'violin' && trace.box.visible && trace.meanline.visible)) ? Lib.identity : []);
    const meanEnter = paths.enter().append('path')
        .attr('class', 'mean')
        .style('fill', 'none')
        .style('vector-effect', 'non-scaling-stroke');
    paths.exit().remove();
    paths.merge(meanEnter).each(function (d) {
        const lcenter = posAxis.c2l(d.pos + bPos, true);
        const pos0 = posAxis.l2p(lcenter - bdPos0) + bPosPxOffset;
        const pos1 = posAxis.l2p(lcenter + bdPos1) + bPosPxOffset;
        const posc = posHasRangeBreaks ? (pos0 + pos1) / 2 : posAxis.l2p(lcenter) + bPosPxOffset;
        ...
        if (trace.orientation === 'h') {
            select(this).attr('d', 'M' + m + ',' + pos0 + 'V' + pos1 +
                (mode === 'sd' ?
                    'm0,0L' + sl + ',' + posc + 'L' + m + ',' + pos0 + 'L' + sh + ',' + posc + 'Z' :
                    ''));
        }
        else {
            select(this).attr('d', 'M' + pos0 + ',' + m + 'H' + pos1 +
                (mode === 'sd' ?
