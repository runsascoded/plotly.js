...
function hoverOnBoxes(pointData, xval, yval, hovermode) {
    ...
    let pLetter, vLetter, pAxis, vAxis, vVal, pVal, dx, dy, dPos, hoverPseudoDistance, spikePseudoDistance;
    const boxDelta = t.bdPos;
    let boxDeltaPos, boxDeltaNeg;
    const posAcceptance = t.wHover;
    const shiftPos = (di) => { return pAxis.c2l(di.pos) + t.bPos - pAxis.c2l(pVal); };
    ...
    if (Color.opacity(lc) && trace.line.width)
        pointData.color = lc;
    else if (Color.opacity(mc) && trace.boxpoints)
        pointData.color = mc;
    else
        pointData.color = trace.fillcolor;
    ...
    const attrs = (hasFences && hasMean) ? ['max', 'uf', 'q3', 'med', 'mean', 'q1', 'lf', 'min'] :
