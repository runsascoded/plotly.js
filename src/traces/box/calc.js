...
export default function calc(gd, trace) {
    ...
    if (trace._hasPreCompStats) {
        ...
        const d2c = (k) => { return valAxis.d2c((trace[k] || [])[i]); };
        let minVal = Infinity;
        let maxVal = -Infinity;
        ...
        trace._extremes[valAxis._id] = Axes.findExtremes(valAxis, [minVal, maxVal], { padded: true });
    }
    else {
        ...
        if (trace.notched && Lib.isTypedArray(valArray))
            valArray = Array.from(valArray);
        trace._extremes[valAxis._id] = Axes.findExtremes(valAxis, trace.notched ? valArray.concat([minLowerNotch, maxUpperNotch]) : valArray, { padded: true });
    }
...
function getPosArrays(trace, posLetter, posAxis, num) {
    ...
    else if ('name' in trace && (posAxis.type === 'category' || (isNumeric(trace.name) &&
        ['linear', 'log'].indexOf(posAxis.type) !== -1) || (Lib.isDateTime(trace.name) &&
        posAxis.type === 'date'))) {
        pos0 = trace.name;
    }
    else {
