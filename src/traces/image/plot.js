...
export default function plot(gd, plotinfo, cdimage, imageLayer) {
    ...
    Lib.makeTraceGroups(imageLayer, cdimage, 'im').each(function (cd) {
        ...
        const realImage = (((trace.zsmooth === 'fast') || (trace.zsmooth === false && supportsPixelated)) &&
        ...
        function drawMagnifiedPixelsOnCanvas(readPixel) {
            ...
            const ipx = (i) => { return Lib.constrain(Math.round(xa.c2p(x0 + i * dx) - left), 0, imageWidth); };
            const jpx = (j) => { return Lib.constrain(Math.round(ya.c2p(y0 + j * dy) - top), 0, imageHeight); };
            ...
            for (i = 0; i < cd0.w; i++) {
                const ipx0 = ipx(i);
                const ipx1 = ipx(i + 1);
                if (ipx1 === ipx0 || isNaN(ipx1) || isNaN(ipx0))
                    continue;
                for (let j = 0; j < cd0.h; j++) {
                    const jpx0 = jpx(j);
                    const jpx1 = jpx(j + 1);
                    if (jpx1 === jpx0 || isNaN(jpx1) || isNaN(jpx0) || !readPixel(i, j))
                        continue;
