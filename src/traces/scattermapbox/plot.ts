...
function ScatterMapbox(this: any, subplot: PlotInfo, uid, clusterEnabled, isHidden) {
...
export default function createScatterMapbox(subplot: PlotInfo, calcTrace) {
    const trace = calcTrace[0].trace;
    const hasCluster = trace.cluster && trace.cluster.enabled;
    const isHidden = trace.visible !== true;

    const scatterMapbox = new ScatterMapbox(
        subplot,
        trace.uid,
        hasCluster,
        isHidden
