...
function ScatterMapbox(this: any, subplot: PlotInfo, uid: any, clusterEnabled: any, isHidden: any) {
...
export default function createScatterMapbox(subplot: PlotInfo, calcTrace: any) {
    const trace = calcTrace[0].trace;
    const hasCluster = trace.cluster && trace.cluster.enabled;
    const isHidden = trace.visible !== true;

    // @ts-ignore TS7009
    const scatterMapbox: any = new ScatterMapbox(
        subplot,
        trace.uid,
        hasCluster,
        isHidden
