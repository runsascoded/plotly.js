...
function convertTextStyle(gd, trace) {
    ...
    if (texttemplate) {
        ...
        for (i = 0; i < N; i++) {
            ...
            optsOut.text.push(Lib.texttemplateString({
...
function convertMarkerStyle(gd, trace) {
    ...
    let isOpen;
    if (!multiSymbol)
        isOpen = helpers.isOpenSymbol(optsIn.symbol);
    // prepare colors
    if (multiSymbol || multiColor || multiLineColor || multiOpacity || multiAngle) {
        ...
        let colors = formatColor(optsIn, optsIn.opacity, count);
        let borderColors = formatColor(optsIn.line, optsIn.opacity, count);
...
function convertMarkerSelection(gd, trace, target) {
    ...
    if (target.marker && target.marker.symbol) {
        ...
        if (target.marker.size)
            optsOut.size = target.marker.size;
        if (target.marker.color)
            optsOut.colors = target.marker.color;
        if (target.marker.opacity !== undefined)
            optsOut.opacity = target.marker.opacity;
    }
    return optsOut;
}
...
function convertLinePositions(gd, trace, positions) {
    ...
    const join = hasNaN || linePositions.length > constants.TOO_MANY_POINTS
        ? 'rect'
...
function convertErrorBarPositions(gd, trace, positions, x, y) {
    ...
    function convertOneAxis(coords, ax) {
        ...
        if (opts && opts.visible && (ax.type === 'linear' || ax.type === 'log')) {
            ...
            const eOffset = { x: [0, 1, 2, 3], y: [2, 3, 0, 1] }[axLetter];
