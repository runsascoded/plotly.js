...
function plot(gd, subplot, cdata) {
    ...
    if (scene.dirty) {
        ...
        if (scene.fill2d) {
            scene.fillOptions = scene.fillOptions.map((fillOptions, i) => {
                const cdscatter = cdata[i];
                if (!fillOptions || !cdscatter || !cdscatter[0] || !cdscatter[0].trace)
                    return;
    ...
    if (scene.glText) {
        scene.glText.forEach((text) => { text.update(vpRange0); });
    }
}
