    ...
    if (hovermode === 'x') {
        ...
        for (i = 0; i < ids.length; i++) {
            ...
            if (xPeriod) {
                ...
                dx = (xpx >= Math.min(x0, x1) &&
                    xpx <= Math.max(x0, x1)) ? 0 : Infinity;
            }
            if (dx < minDist) {
                minDist = dx;
                pty = y[k];
                ...
                if (yPeriod) {
                    ...
                    dy = (ypx >= Math.min(y0, y1) &&
                        ypx <= Math.max(y0, y1)) ? 0 : Infinity;
                }
...
function calcHover(pointData, x, y, trace) {
    ...
    if (di.htx)
        pointData2.text = di.htx;
    else if (di.tx)
        pointData2.text = di.tx;
    else if (trace.text)
        pointData2.text = trace.text;
