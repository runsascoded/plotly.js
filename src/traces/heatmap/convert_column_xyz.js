...
export default function convertColumnData(trace, ax1, ax2, var1Name, var2Name, arrayVarNames) {
    ...
    for (i = 0; i < colLen; i++) {
        if (col1[i] !== BADNUM && col2[i] !== BADNUM) {
            ...
            if (hasColumnText)
                text[i2][i1] = textCol[i];
            if (hasColumnHoverText)
                hovertext[i2][i1] = hoverTextCol[i];
        }
    }
    ...
    if (hasColumnText)
        trace._text = text;
    if (hasColumnHoverText)
        trace._hovertext = hovertext;
    if (ax1 && ax1.type === 'category') {
        trace['_' + var1Name + 'CategoryMap'] = col1vals.map((v) => ax1._categories[v]);
    }
    if (ax2 && ax2.type === 'category') {
        trace['_' + var2Name + 'CategoryMap'] = col2vals.map((v) => ax2._categories[v]);
    }
