...
export default function getBinSpanLabelRound(leftGap, rightGap, binEdges, pa, calendar) {
    ...
    const leftDigit = Math.min(biggestDigitChanged(edge0 + dv1, edge0 + dv2, pa, calendar), biggestDigitChanged(edge1 + dv1, edge1 + dv2, pa, calendar));
    const rightDigit = Math.min(biggestDigitChanged(edge0 + dv0, edge0 + dv1, pa, calendar), biggestDigitChanged(edge1 + dv0, edge1 + dv1, pa, calendar));
...
function biggestDigitChanged(v1, v2, pa, calendar) {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        if (digit === nextDigit)
            break;
        if (didDigitChange(nextDigit, v1, v2, isDate, pa, calendar))
            digit = nextDigit;
        else
            break;
    }
