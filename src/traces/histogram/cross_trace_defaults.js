...
export default function crossTraceDefaults(fullData, fullLayout) {
    ...
    for (i = 0; i < histTraces.length; i++) {
        ...
        if (!traceIs(traceOut, '2dMap')) {
            ...
            if (!groupName && fullLayout.barmode !== 'overlay') {
                groupName = (getAxisGroup(fullLayout, traceOut.xaxis) +
    ...
    for (i = 0; i < otherTracesList.length; i++) {
        ...
        if (traceIs(traceOut, '2dMap')) {
            for (k = 0; k < 2; k++) {
                ...
                const binGroupInDir = coerce(binDir + 'bingroup', binGroup ? binGroup + '__' + binDir : null);
    ...
    for (groupName in allBinOpts) {
        ...
        for (j = 0; j < BINATTRS.length; j++) {
            ...
            if (attr === 'nbins' && binOpts.sizeFound)
                continue;
            for (i = 0; i < traces.length; i++) {
                traceOut = traces[i];
