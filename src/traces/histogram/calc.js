...
function calc(gd, trace) {
    ...
    let isAvg = false;
    const pr2c = (v) => { return pa.r2c(v, 0, calendar); };
    let rawCounterData;
    ...
    if (!gd._fullLayout._roundFnOpts)
        gd._fullLayout._roundFnOpts = {};
    const groupName = trace['_' + mainData + 'bingroup'];
    let roundFnOpts = { leftGap: Infinity, rightGap: Infinity };
    if (groupName) {
...
function calcAllAutoBins(gd, trace, pa, mainData, _overlayEdgeCase) {
    ...
    const r2c = (v) => { return pa.r2c(v, 0, calendar); };
    const c2r = (v) => { return pa.c2r(v, 0, calendar); };
    ...
    if ((binOpts.startFound || hasStart) && startIn !== pa.r2l(mainStart)) {
        ...
        if (!hasStart)
            Lib.nestedProperty(trace, binAttr + '.start').set(traceBinOptsCalc.start);
    }
    ...
    if ((binOpts.endFound || hasEnd) && endIn !== pa.r2l(mainEnd)) {
        ...
        if (!hasEnd)
            Lib.nestedProperty(trace, binAttr + '.start').set(traceBinOptsCalc.end);
    }
...
function handleSingleValueOverlays(gd, trace, pa, mainData, binAttr) {
    ...
    for (i = 0; i < singleValuedTraces.length; i++) {
        ...
        binOpts = fullLayout._histogramBinOpts[tracei['_' + mainData + 'bingroup']];
        if (binOpts)
            Lib.extendFlat(binOpts, newBins);
    }
