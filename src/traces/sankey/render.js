...
function sankeyModel(layout, d, traceIndex) {
        ...
        for (i = 0; i < graph.nodes.length; i++) {
            ...
            for (j = 0; j < keys.length; j++) {
                ...
                for (k = 0; k < flowLinks.length; k++) {
                    link = flowLinks[k];
                    if (!totalPerLabel[link.label])
                        totalPerLabel[link.label] = 0;
    ...
    function snapToColumns(nodes) {
        ...
        const orderedNodes = nodes.map((n, i) => {
            return {
                x0: n.x0,
                index: i
            };
...
function attachPointerEvents(selection, sankey, eventSet) {
    selection
        ...
        .on('mousemove.basic', function (event, d) {
        if (!d.interactionState.dragInProgress && !d.partOfGroup) {
        ...
        .on('click.basic', function (event, d) {
...
function attachForce(sankeyNode, forceKey, d, gd) {
    ...
    const nodes = d.graph.nodes
        .filter((n) => n.originalX === d.node.originalX)
        // Filter out children
        .filter((n) => !n.partOfGroup);
...
function persistFinalNodePositions(d, gd) {
    ...
    _guiRestyle(gd, {
        ...
        .then(() => {
        if (gd._fullLayout._dragCover)
            gd._fullLayout._dragCover.remove();
    });
}
...
export default function (gd, svg, calcData, layout, callbacks) {
    ...
    sankeyMerged.each(function (d, i) {
    ...
    nodeLabel.merge(nodeLabelEnter)
        ...
        const blockHeight = d.textFont.size * ((nLines - 1) * LINE_SPACING - CAP_SHIFT);
        ...
        const flipText = d.horizontal ? '' : ('scale(-1,1)' + strRotate(90));
