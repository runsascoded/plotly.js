...
function ownTrace(selection, d) {
    return selection.filter((s) => s.key === d.traceId);
}
...
export default function plot(gd, calcData) {
    ...
    for (let i = 0; i < gd._fullData.length; i++) {
        if (!gd._fullData[i].visible)
            continue;
        if (gd._fullData[i].type !== cn.sankey)
            continue;
    ...
    const linkSelect = (element, d) => {
        const evt = d.link;
        evt.originalEvent = event;
        gd._hoverdata = [evt];
        Fx.click(gd, { target: true });
    };
    const linkHover = (element, d, sankey) => {
        if (gd._fullLayout.hovermode === false)
            return;
    ...
    const linkHoverFollow = function (element, d) {
        ...
        if (obj.hoverinfo === 'none' || obj.hoverinfo === 'skip')
            return;
        const hoverItems = [];
        ...
        for (let i = 0; i < d.flow.links.length; i++) {
            ...
            if (gd._fullLayout.hovermode === 'closest' && d.link.pointNumber !== link.pointNumber)
                continue;
            if (d.link.pointNumber === link.pointNumber)
                anchorIndex = i;
    ...
    const linkUnhover = (element, d, sankey) => {
        if (gd._fullLayout.hovermode === false)
            return;
    ...
    const nodeSelect = (element, d, sankey) => {
        const evt = d.node;
        evt.originalEvent = event;
        gd._hoverdata = [evt];
    ...
    const nodeHover = (element, d, sankey) => {
        if (gd._fullLayout.hovermode === false)
            return;
    ...
    const nodeHoverFollow = (element, d) => {
        if (gd._fullLayout.hovermode === false)
            return;
    ...
    const nodeUnhover = (element, d, sankey) => {
        if (gd._fullLayout.hovermode === false)
            return;
        select(element).call(nodeNonHoveredStyle, d, sankey);
        if (d.node.trace.node.hoverinfo !== 'skip') {
