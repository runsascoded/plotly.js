...
export default function supplyDefaults(traceIn, traceOut, defaultColor, layout) {
    ...
    function readComponents(array) {
        const ret = array.map((attr) => {
            const result = coerce(attr);
            if (result && Lib.isArrayOrTypedArray(result))
                return result;
            return null;
        });
    ...
    if ((traceOut.i && (!traceOut.j || !traceOut.k)) ||
    ...
    if ('intensity' in traceIn) {
        ...
        if ('facecolor' in traceIn)
            coerce('facecolor');
        else if ('vertexcolor' in traceIn)
            coerce('vertexcolor');
        else
