...
export default function plotOne(gd, cd, element, transitionOpts, drawDescendants) {
    ...
    const findClosestEdge = (pt, ref, size) => {
        const e = trace.tiling.pad;
        const isLeftOfRect = (x) => { return x - e <= ref.x0; };
        const isRightOfRect = (x) => { return x + e >= ref.x1; };
        const isBottomOfRect = (y) => { return y - e <= ref.y0; };
        const isTopOfRect = (y) => { return y + e >= ref.y1; };
    ...
    let prevEntry = null;
    const prevLookupPathbar = {};
    const prevLookupSlices = {};
    let nextOfPrevEntry = null;
    const getPrev = (pt, onPathbar) => {
        return onPathbar ?
            prevLookupPathbar[getKey(pt)] :
            prevLookupSlices[getKey(pt)];
    };
    const getOrigin = (pt, onPathbar, refRect, size) => {
        if (onPathbar) {
    ...
    const viewMapX = (x) => { return cenX + x; };
    const viewMapY = (y) => { return cenY + y; };
    const barY0 = viewMapY(0);
    const barX0 = viewMapX(0);
    const viewBarX = (x) => { return barX0 + x; };
    const viewBarY = (y) => { return barY0 + y; };
    function pos(x, y) {
        return x + ',' + y;
    }
    const xStart = viewBarX(0);
    const limitX0 = (p) => { p.x = Math.max(xStart, p.x); };
    const edgeshape = trace.pathbar.edgeshape;
    // pathbar(directory) path generation fn
    const pathAncestor = (d) => {
    ...
    const pathDescendant = (d) => {
        ...
        const arc = (rx, ry) => { return r ? 'a' + pos(r, r) + ' 0 0 1 ' + pos(rx, ry) : ''; };
        return ('M' + pos(_x0, _y0 + r) +
            arc(r, -r) +
            'L' + pos(_x1 - r, _y0) +
            arc(r, r) +
            'L' + pos(_x1, _y1 - r) +
    ...
    const toMoveInsideSlice = (pt, opts) => {
        let x0 = pt.x0;
        let x1 = pt.x1;
        let y0 = pt.y0;
        let y1 = pt.y1;
    ...
    const interpFromParent = (pt, onPathbar) => {
        let parentPrev;
        let i = 0;
        let Q = pt;
