...
export default function partition(entry, size, opts) {
    ...
    const result = d3Hierarchy
        ...
        .size(swapXY ? [size[1], size[0]] : size)(entry);
...
function transposedSquarifyRatio(ratio, parent, x0, y0, x1, y1) {
    ...
    while (i0 < n) {
        ...
        if (row.dice)
            d3Hierarchy.treemapDice(row, x0, y0, x1, value ? y0 += dy * sumValue / value : y1);
        else
            d3Hierarchy.treemapSlice(row, x0, y0, value ? x0 += dx * sumValue / value : x1, y1);
        value -= sumValue;
        i0 = i1;
    }
}
