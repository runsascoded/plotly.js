...
export default function plot(gd, plotinfo, cdscatter, scatterLayer, transitionOpts, makeOnCompleteCallback) {
    let join, onComplete;
...
function createFills(gd, traceJoin, plotinfo) {
    traceJoin.each(function (d) {
        ...
        fillJoin.exit()
            ...
            .remove();
        fillJoin.merge(fillJoinEnter).order().each(function (d) {
...
function plotOne(gd, idx, plotinfo, cdscatter, cdscatterAll, element, transitionOpts) {
    ...
    const tr = select(element);
    const errorBarGroup = ensureSingle(tr, 'g', 'errorbars');
    ...
    let prevRevpath = '';
    let prevPolygons = [];
    const prevtrace = trace._prevtrace;
    let prevFillsegments = null;
    let prevFillElement = null;
    if (prevtrace) {
    ...
    if (subTypes.hasLines(trace) || trace.fill !== 'none') {
        ...
        if (['hv', 'vh', 'hvh', 'vhv'].indexOf(line.shape) !== -1) {
            ...
            revpathbase = steps(line.shape.split('').reverse().join(''));
        }
    ...
    const lineJoin = lines.selectAll('.js-line').data(segments);
    ...
    setClipUrl(lineJoin.merge(lineJoinEnter), plotinfo.layerClipId, gd);
    ...
    const makePolygonsToPrevious = (prevFillsegments) => {
        let polygons, i;
        if (!prevFillsegments || prevFillsegments.length === 0) {
            ...
            for (i = 0; i < fillsegments.length; i++) {
                ...
                const zeropoints = [pt1, pt0];
                const polypoints = zeropoints.concat(fillsegments[i]);
            ...
            const reversedPrevFillsegment = prevFillsegments[prevFillsegments.length - 1].slice();
    ...
    function visFilter(d) {
        return d.filter((v) => !v.gap && v.vis);
    }
    function visFilterWithGaps(d) {
        return d.filter((v) => v.vis);
    }
    function gapFilter(d) {
        return d.filter((v) => !v.gap);
    }
    function keyFunc(d) {
        return d.id;
    }
    ...
    function makePoints(points, text, cdscatter) {
        ...
        if (showMarkers || showText) {
            ...
            const isInferZero = stackGroup && (gd._fullLayout._scatterStackOpts[xa._id + ya._id][stackGroup].stackgaps === 'infer zero');
            ...
            if (showMarkers)
                markerFilter = showFilter;
            if (showText)
                textFilter = showFilter;
        }
        // marker points
...
function selectMarkers(gd, idx, plotinfo, cdscatter, cdscatterAll) {
    const xa = plotinfo.xaxis;
    const ya = plotinfo.yaxis;
    const xr = extent(simpleMap(xa.range, xa.r2c));
    const yr = extent(simpleMap(ya.range, ya.r2c));
    const trace = cdscatter[0].trace;
