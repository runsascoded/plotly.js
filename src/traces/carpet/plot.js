...
function drawGridLines(xaxis, yaxis, layer, axis, axisLetter, gridlines, _isMajor, isStatic) {
    ...
    gridJoin.merge(gridJoinEnter).each(function (d) {
        const gridline = d;
        const x = gridline.x;
        const y = gridline.y;
...
function drawAxisLabels(gd, xaxis, yaxis, trace, t, layer, labels, labelClass) {
    ...
    labelJoin.merge(labelJoinEnter).each(function (label, i) {
...
function drawAxisTitle(gd, layer, trace, t, xy, dxy, axis, xa, ya, labelOrientation, labelClass) {
    ...
    titleJoin.merge(titleJoinEnter).each(function () {
        ...
        el.attr('transform', strTranslate(orientation.p[0], orientation.p[1]) +
            strRotate(orientation.angle) +
