...
function handleMarkerDefaults(traceIn, traceOut, layout, coerce, isPie) {
    ...
    if (traceIn.marker && !traceOut.marker.pattern.fgcolor)
        traceOut.marker.pattern.fgcolor = traceIn.marker.colors;
    if (!traceOut.marker.pattern.bgcolor)
        traceOut.marker.pattern.bgcolor = layout.paper_bgcolor;
}
function supplyDefaults(traceIn, traceOut, defaultColor, layout) {
    ...
    let textInfo;
    if (!textTemplate)
        textInfo = coerce('textinfo', Lib.isArrayOrTypedArray(textData) ? 'text+percent' : 'percent');
    coerce('hovertext');
    ...
    if (title) {
        ...
        if (!hole && titlePosition === 'middle center')
            traceOut.title.position = 'top center';
