...
export function getFirstFilled(array: any[], indices: number[]): any {
...
export function castOption(item: any, indices: number[]): any {
    if(isArrayOrTypedArray(item)) return getFirstFilled(item, indices);
    else if(item) return item;
}

export function getRotationAngle(rotation: number | string): number {
