...
function plot(gd, cdModule) {
    ...
    const plotGroups = Lib.makeTraceGroups(fullLayout._pielayer, cdModule, 'trace').each(function (cd) {
        ...
        plotGroup.each(function () {
            ...
            slicesMerged.each(function (pt, i) {
                if (pt.hidden) {
                ...
                sliceTextGroup.merge(sliceTextGroupEnter).each(function () {
                    const sliceText = Lib.ensureSingle(select(this), 'text', '', function (s) {
                        // prohibit tex interpretation until we can handle
                        // tex and regular text together
                        s.attr('data-notex', 1);
                    });
                    const font = Lib.ensureUniformFontSize(gd, textPosition === 'outside'
            ...
            titleTextGroup.merge(titleTextGroupEnter).each(function () {
                ...
                titleText.attr('transform', strTranslate(transform.x, transform.y) +
...
function attachFxHandlers(sliceTop, gd, cd) {
    ...
    if (!('_hasHoverLabel' in trace))
        trace._hasHoverLabel = false;
    // have we emitted a hover event, so later an unhover event should be emitted
    // note that click events do not depend on this - you can still get them
    // with hovermode: false or if you were earlier dragging, then clicked
    // in the same slice that you moused up in
    if (!('_hasHoverEvent' in trace))
        trace._hasHoverEvent = false;
    sliceTop.on('mouseover', function (event, pt) {
        ...
        if (trace2.hovertemplate || (hoverinfo !== 'none' && hoverinfo !== 'skip' && hoverinfo)) {
            ...
            const text = [];
            if (hoverinfo && hoverinfo.indexOf('label') !== -1)
                text.push(pt.label);
            pt.text = helpers.castOption(trace2.hovertext || trace2.text, pt.pts);
            if (hoverinfo && hoverinfo.indexOf('text') !== -1) {
                const tx = pt.text;
                if (Lib.isValidTextValue(tx))
                    text.push(tx);
            }
            pt.value = pt.v;
    ...
    sliceTop.on('click', function (event, pt) {
...
function determineOutsideTextFont(trace, pt, layoutFont) {
    const color = helpers.castOption(trace.outsidetextfont.color, pt.pts) ||
        helpers.castOption(trace.textfont.color, pt.pts) ||
        layoutFont.color;
    const family = helpers.castOption(trace.outsidetextfont.family, pt.pts) ||
        helpers.castOption(trace.textfont.family, pt.pts) ||
        layoutFont.family;
    const size = helpers.castOption(trace.outsidetextfont.size, pt.pts) ||
        helpers.castOption(trace.textfont.size, pt.pts) ||
        layoutFont.size;
    const weight = helpers.castOption(trace.outsidetextfont.weight, pt.pts) ||
        helpers.castOption(trace.textfont.weight, pt.pts) ||
        layoutFont.weight;
    const style = helpers.castOption(trace.outsidetextfont.style, pt.pts) ||
        helpers.castOption(trace.textfont.style, pt.pts) ||
        layoutFont.style;
    const variant = helpers.castOption(trace.outsidetextfont.variant, pt.pts) ||
        helpers.castOption(trace.textfont.variant, pt.pts) ||
        layoutFont.variant;
    const textcase = helpers.castOption(trace.outsidetextfont.textcase, pt.pts) ||
        helpers.castOption(trace.textfont.textcase, pt.pts) ||
        layoutFont.textcase;
    const lineposition = helpers.castOption(trace.outsidetextfont.lineposition, pt.pts) ||
        helpers.castOption(trace.textfont.lineposition, pt.pts) ||
        layoutFont.lineposition;
    const shadow = helpers.castOption(trace.outsidetextfont.shadow, pt.pts) ||
...
function determineInsideTextFont(trace, pt, layoutFont) {
    ...
    const family = helpers.castOption(trace.insidetextfont.family, pt.pts) ||
        helpers.castOption(trace.textfont.family, pt.pts) ||
        layoutFont.family;
    const size = helpers.castOption(trace.insidetextfont.size, pt.pts) ||
        helpers.castOption(trace.textfont.size, pt.pts) ||
        layoutFont.size;
    const weight = helpers.castOption(trace.insidetextfont.weight, pt.pts) ||
        helpers.castOption(trace.textfont.weight, pt.pts) ||
        layoutFont.weight;
    const style = helpers.castOption(trace.insidetextfont.style, pt.pts) ||
        helpers.castOption(trace.textfont.style, pt.pts) ||
        layoutFont.style;
    const variant = helpers.castOption(trace.insidetextfont.variant, pt.pts) ||
        helpers.castOption(trace.textfont.variant, pt.pts) ||
        layoutFont.variant;
    const textcase = helpers.castOption(trace.insidetextfont.textcase, pt.pts) ||
        helpers.castOption(trace.textfont.textcase, pt.pts) ||
        layoutFont.textcase;
    const lineposition = helpers.castOption(trace.insidetextfont.lineposition, pt.pts) ||
        helpers.castOption(trace.textfont.lineposition, pt.pts) ||
        layoutFont.lineposition;
    const shadow = helpers.castOption(trace.insidetextfont.shadow, pt.pts) ||
        helpers.castOption(trace.textfont.shadow, pt.pts) ||
        layoutFont.shadow;
    return {
...
function getInscribedRadiusFraction(pt, cd0) {
    if (pt.v === cd0.vTotal && !cd0.trace.hole)
        return 1; // special case of 100% with no hole
...
function getMaxPull(trace) {
    ...
    if (Lib.isArrayOrTypedArray(maxPull)) {
        ...
        for (j = 0; j < trace.pull.length; j++) {
            if (trace.pull[j] > maxPull)
                maxPull = trace.pull[j];
        }
    }
    return maxPull;
}
function scootLabels(quadrants, trace) {
    let xHalf, yHalf, equatorFirst, farthestX, farthestY, xDiffSign, yDiffSign, thisQuad, oppositeQuad, wholeSide, i, thisQuadOutside, firstOppositeOutsidePt;
    ...
    function scootOneLabel(thisPt, prevPt) {
        ...
        if (newExtraY * yDiffSign > 0)
            thisPt.labelExtraY = newExtraY;
        // make sure this label doesn't overlap any slices
        if (!Lib.isArrayOrTypedArray(trace.pull))
            return; // this can only happen with array pulls
        for (i = 0; i < wholeSide.length; i++) {
            ...
            if ((thisPt.pxmid[1] - otherPt.pxmid[1]) * yDiffSign > 0) {
                ...
                if (newExtraX * xDiffSign > 0)
                    thisPt.labelExtraX += newExtraX;
            }
        }
    }
    for (yHalf = 0; yHalf < 2; yHalf++) {
        ...
        for (xHalf = 0; xHalf < 2; xHalf++) {
            ...
            for (i = 0; i < thisQuadOutside.length; i++) {
                ...
                if (firstOppositeOutsidePt && !i)
                    prevPt = firstOppositeOutsidePt;
...
function setCoords(cd) {
    ...
    if (trace.direction === 'counterclockwise') {
        for (i = 0; i < cd.length; i++) {
            if (!cd[i].hidden)
                break; // find the first non-hidden slice
        }
        if (i === cd.length)
            return; // all slices hidden
