...
export default function setContours(trace, vals) {
    ...
    if (trace.autocontour) {
        ...
        const dummyAx = autoContours(zmin, zmax, trace.ncontours);
        contours.size = dummyAx.dtick;
        contours.start = Axes.tickFirst(dummyAx);
        dummyAx.range.reverse();
        contours.end = Axes.tickFirst(dummyAx);
        if (contours.start === zmin)
            contours.start += contours.size;
        if (contours.end === zmax)
            contours.end -= contours.size;
        ...
        if (!trace._input.contours)
            trace._input.contours = {};
        ...
        if (!(contours.size > 0)) {
            ...
            if (start === end)
                ...
                sizeOut = autoContours(start, end, trace.ncontours).dtick;
