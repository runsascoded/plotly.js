...
export function plot(gd, plotinfo, cdcontours, contourLayer) {
...
function makeLinesAndLabels(plotgroup, pathinfo, gd, cd0, contours) {
    ...
    if (showLabels) {
        ...
        linegroup.each(function (d) {
            ...
            select(this).selectAll('path').each(function () {
                ...
                for (let i = 0; i < maxLabels; i++) {
                    const loc = findBestTextLocation(path, pathBounds, textOpts, labelData, bounds);
                    if (!loc)
                        break;
        ...
        dummyText.remove();
        drawLabels(labelGroupMerged, labelData, gd, lineClip, clipLinesForLabels ? labelClipPathData : null);
    }
    if (showLabels && !showLines)
        linegroup.remove();
}
export function createLines(lineContainer, makeLines, pathinfo, isStatic) {
...
export function createLineClip(lineContainer, clipLinesForLabels, gd, uid) {
...
export function labelFormatter(gd, cd0) {
...
export function calcTextOpts(level, contourFormat, dummyText, gd) {
...
export function findBestTextLocation(path, pathBounds, textOpts, labelData, plotBounds) {
    ...
    for (let j = 0; j < costConstants.ITERATIONS; j++) {
        ...
        if (cost > costConstants.MAXCOST * 2)
            break;
...
function locationCost(loc, textOpts, labelData, bounds) {
    ...
    for (let i = 0; i < labelData.length; i++) {
        ...
        const dist = Lib.segmentDistance(x1, y1, x2, y2, labeli.x - dxd, labeli.y - dyd, labeli.x + dxd, labeli.y + dyd) * 2 / (textOpts.height + labeli.height);
...
export function addLabelData(loc, textOpts, labelData, labelClipPathData) {
...
export function drawLabels(labelGroup, labelData, gd, lineClip, labelClipPathData) {
...
function clipGaps(plotGroup, plotinfo, gd, cd0, perimeter) {
    ...
    if (trace.connectgaps === false) {
        ...
        path.attr('d', (clipPathInfo.prefixBoundary ? 'M' + perimeter.join('L') + 'Z' : '') +
