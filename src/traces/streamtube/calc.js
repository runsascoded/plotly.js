...
function processGrid(trace) {
    ...
    if (!len || len < Xs.length * Ys.length * Zs.length)
        empty();
    const getArray = (c) => { return c === 'x' ? x : c === 'y' ? y : z; };
    const getVals = (c) => { return c === 'x' ? Xs : c === 'y' ? Ys : Zs; };
    const getDir = (c) => { return c[len - 1] < c[0] ? -1 : 1; };
    ...
    for (i = 0; i < ni - 1; i++) {
        for (j = 0; j < nj - 1; j++) {
            for (k = 0; k < nk - 1; k++) {
                ...
                if (!(arrK[q000] * dirK < arrK[q001] * dirK) ||
