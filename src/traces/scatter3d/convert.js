...
function calculateErrorParams(errors) {
    ...
    for (let i = 0; i < 3; i++) {
        let e = errors[i];
        if (e && e.copy_zstyle !== false && errors[2].visible !== false)
            e = errors[2];
        if (!e || !e.visible)
            continue;
...
function calculateTextOffset(tp) {
    ...
    const textOffset = [defaultAlignmentX, defaultAlignmentY];
    if (Array.isArray(tp)) {
...
function formatParam(paramIn, len, calculate, dflt, extraFn) {
    ...
    if (Lib.isArrayOrTypedArray(paramIn)) {
        paramOut = [];
        for (let i = 0; i < len; i++) {
            if (paramIn[i] === undefined)
                paramOut[i] = dflt;
            else
                paramOut[i] = calculate(paramIn[i], extraFn);
        }
    }
    else
...
function _arrayToColor(color) {
    if (Lib.isArrayOrTypedArray(color)) {
        const c = color[0];
        if (Lib.isArrayOrTypedArray(c))
            color = c;
        return ('rgb(' +
            color.slice(0, 3).map((x) => Math.round(x * 255)) +
            ')');
    }
    return null;
}
...
proto.update = function (data) {
    ...
    if (data.marker && data.marker.opacity !== undefined)
    ...
    if (options.delaunayAxis >= 0) {
        const delaunayOptions = constructDelaunay(options.position, options.delaunayColor, options.delaunayAxis);
