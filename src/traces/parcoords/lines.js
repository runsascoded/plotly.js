...
export default function (canvasGL, d) {
    ...
    const plotGlPixelRatio = Math.max(supportedLineWidth[0], Math.min(supportedLineWidth[1], d.viewModel.plotGlPixelRatio));
    const renderState = {
        currentRafs: {},
        drawCompleted: true,
        clearOnly: false
    ...
    function update(dNew) {
        ...
        const color = isPick ? lines.color.map((_, i) => i / lines.color.length) : lines.color;
    ...
    function makeConstraints(isContext) {
        ...
        if (!isContext) {
            for (i = 0; i < initialDims.length; i++) {
                ...
                if (ranges.length < 2)
                    continue; // bail if the bounding box based filter is sufficient
        ...
        if (maskTexture)
            maskTexture(textureData);
        else
            maskTexture = regl.texture(textureData);
        return {
    ...
    function renderGLParcoords(panels, setChanged, clearOnly) {
        ...
        for (i = 0; i < panelCount; i++) {
            ...
            if (setChanged ||
                ...
                const item = makeItem(model, leftmost, rightmost, i, i0, i1, x, y, p.panelSizeX, p.panelSizeY, p.dim0.crossfilterDimensionIndex, isContext ? 0 : isPick ? 2 : 1, constraints, plotGlPixelRatio);
