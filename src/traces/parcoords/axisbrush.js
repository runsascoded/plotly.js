...
function ordinalScaleSnap(isHigh, a, v, existingRanges) {
    if (overlappingExisting(v, existingRanges))
        return v;
    ...
    for (let i = first; dir * i < dir * last; i += dir) {
        ...
        if (dir * v < dir * closeToCovering(aHere, aNext))
            return snapOvershoot(aHere, aPrev);
        if (dir * v < dir * aNext || nextI === last)
            return snapOvershoot(aNext, aHere);
        aPrev = aHere;
        aHere = aNext;
    }
}
function overlappingExisting(v, existingRanges) {
    for (let i = 0; i < existingRanges.length; i++) {
        if (v >= existingRanges[i][0] && v <= existingRanges[i][1])
            return true;
    }
    return false;
}
...
function getRegion(fPix, y) {
    ...
    if (y > fPix[1] + pad || y < fPix[0] - pad)
        return;
    if (y >= 0.9 * fPix[1] + 0.1 * fPix[0])
        return 'n';
    if (y <= 0.9 * fPix[0] + 0.1 * fPix[1])
        return 's';
    return 'ns';
}
...
function cleanRanges(ranges, dimension) {
    if (Array.isArray(ranges[0])) {
        ...
        if (!dimension.multiselect)
            ...
            ranges = dedupeRealRanges(ranges.sort(startAsc));
    }
    else
        ranges = [ranges.sort(sortAsc)];
    // ordinal snapping
