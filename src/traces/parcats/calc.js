...
export default function calc(gd, trace) {
    ...
    if (visibleDims.length === 0)
        return [];
    const uniqueInfoDims = visibleDims.map((dim) => {
        let categoryValues;
    ...
    const categoryIndsDims = uniqueInfoDims.map((di) => di.inds);
    ...
    const dimensionModels = visibleDims.map((di, i) => createDimensionModel(i, di._index, di._displayindex, di.label, totalCount));
    for (valueInd = 0; valueInd < numValues; valueInd++) {
        count = counts[valueInd % counts.length];
...
function getUniqueInfo(values, uniqueValues) {
    ...
    if (uniqueValues === undefined || uniqueValues === null) {
        ...
        uniqueValues = uniqueValues.map((e) => e);
    }
    // Initialize Variables
    const uniqueValueCounts = {};
    const uniqueValueInds = {};
    const inds = [];
    // Initialize uniqueValueCounts and
    uniqueValues.forEach((uniqueVal, valInd) => {
    ...
    // Build UniqueInfo
    const uniqueCounts = uniqueValues.map((v) => uniqueValueCounts[v]);
    return {
...
function validateDimensionDisplayInds(visibleDims) {
    const displayInds = visibleDims.map((d) => d.displayindex);
    let i;
