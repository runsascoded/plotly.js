...
export function calc(gd, trace) {
    ...
    const refs = {};
    const addToLookup = (parent, v) => {
        if (parent2children[parent])
            parent2children[parent].push(v);
        else
            parent2children[parent] = [v];
        refs[v] = 1;
    };
        ...
        switch (trace.branchvalues) {
            ...
            case 'total':
                hierarchy.each(function (d) {
                    ...
                    let v = cdi.v;
                    if (d.children) {
                        const partialSum = d.children.reduce((a, c) => a + c.data.data.v, 0);
...
export function _runCrossTraceCalc(desiredType, gd) {
    const fullLayout = gd._fullLayout;
    ...
    if (fullLayout['extend' + desiredType + 'colors']) {
        colorWay = generateExtendedColors(colorWay, desiredType === 'icicle' ? icicleExtendedColorWays :
...
function countDescendants(node, trace, opts) {
    ...
    if (children) {
        ...
        if (opts.branches)
            nChild++; // count this branch
    }
    else {
        if (opts.leaves)
            nChild++; // count this leaf
    }
    // save to the node
