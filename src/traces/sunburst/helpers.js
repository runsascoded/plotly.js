...
export function findEntryWithLevel(hierarchy, level) {
    let out;
    if (level) {
...
export function findEntryWithChild(hierarchy, childId) {
    let out;
    hierarchy.eachAfter(function (pt) {
...
export function isEntry(pt) {
    return !pt.parent;
}
export function isLeaf(pt) {
    return !pt.children;
}
export function getPtId(pt) {
    return pt.data.data.id;
}
export function getPtLabel(pt) {
    return pt.data.data.label;
}
export function getValue(d) {
    return d.value;
}
export function isHierarchyRoot(pt) {
    return getParentId(pt) === '';
}
export function setSliceCursor(sliceTop, gd, opts) {
    let hide = opts.isTransitioning;
    if (!hide) {
        const pt = sliceTop.datum();
        hide = ((opts.hideOnRoot && isHierarchyRoot(pt)) ||
...
function determineInsideTextFont(trace, pt, layoutFont, opts) {
    ...
    let customColor = Lib.castOption(trace, ptNumber, (onPathbar ? 'pathbar.textfont' : 'insidetextfont') + '.color');
...
export function getInsideTextFontKey(keyStr, trace, pt, layoutFont, opts) {
    const onPathbar = (opts || {}).onPathbar;
    const cont = onPathbar ? 'pathbar.textfont' : 'insidetextfont';
    const ptNumber = pt.data.data.i;
    return (Lib.castOption(trace, ptNumber, cont + '.' + keyStr) ||
        Lib.castOption(trace, ptNumber, 'textfont.' + keyStr) ||
        layoutFont.size);
}
export function getOutsideTextFontKey(keyStr, trace, pt, layoutFont) {
    const ptNumber = pt.data.data.i;
    return (Lib.castOption(trace, ptNumber, 'outsidetextfont.' + keyStr) ||
        Lib.castOption(trace, ptNumber, 'textfont.' + keyStr) ||
        layoutFont.size);
}
export function isOutsideText(trace, pt) {
    return !trace._hasColorscale && isHierarchyRoot(pt);
}
export function determineTextFont(trace, pt, layoutFont, opts) {
...
export function hasTransition(transitionOpts) {
...
export function formatPercent(v, separators) {
    let tx = Lib.formatPercent(v, 0); // use funnel(area) version
    if (tx === '0%')
        tx = pieHelpers.formatPiePercent(v, separators); // use pie version
    return tx;
}
