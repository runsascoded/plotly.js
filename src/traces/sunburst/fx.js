...
export default function attachFxHandlers(sliceTop, entry, gd, cd, opts) {
    ...
    const isTreemapOrIcicle = trace.type === 'treemap' ||
        trace.type === 'icicle';
    // hover state vars
    ...
    const onMouseOver = (event, pt) => {
        const fullLayoutNow = gd._fullLayout;
        if (gd._dragging || fullLayoutNow.hovermode === false)
            return;
        ...
        if (hovertemplate || (hoverinfo && hoverinfo !== 'none' && hoverinfo !== 'skip')) {
            ...
            let parts = [];
            const thisText = [];
            const hasFlag = (flag) => { return parts.indexOf(flag) !== -1; };
            if (hoverinfo) {
                parts = hoverinfo === 'all' ?
            ...
            if (hasFlag('label') && hoverPt.label)
                thisText.push(hoverPt.label);
            if (cdi.hasOwnProperty('v')) {
                hoverPt.value = cdi.v;
                hoverPt.valueLabel = formatValue(hoverPt.value, separators);
                if (hasFlag('value'))
                    thisText.push(hoverPt.valueLabel);
            }
    ...
    const onClick = function (event, pt) {
        ...
        const typeClickEvtData = {
            points: [makeEventData(pt, traceNow, opts.eventDataKeys)],
            event: event
        };
        if (!noTransition)
            typeClickEvtData.nextLevel = nextLevel;
...
function makeEventData(pt, trace, keys) {
    ...
    if ('parentString' in pt && !helpers.isHierarchyRoot(pt))
        out.parent = pt.parentString;
