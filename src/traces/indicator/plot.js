...
function drawNumbers(gd, plotGroup, cd, opts) {
    ...
    function transitionFormat(valueformat, fmt, from, to) {
        ...
        if (valueformat.match('s') && // If using SI prefix
            ...
            return function (v) {
                ...
                if (Math.abs(v) < 1)
                    return Axes.tickText(transitionAx, v).text;
                return fmt(v);
            };
        }
        else {
    ...
    function drawBignumber() {
        ...
        const bignumberFmt = (v) => { return Axes.tickText(bignumberAx, v).text; };
    ...
    function drawDelta() {
        ...
        const deltaFmt = (v) => { return Axes.tickText(deltaAx, v).text; };
        ...
        const deltaFormatText = (value, numberFmt) => {
    ...
    if (trace._hasNumber || trace._hasDelta) {
        numbers.attr('transform', function () {
            ...
            if (numbersAlign === 'center')
                ref = (numbersbBox.left + numbersbBox.right) / 2;
