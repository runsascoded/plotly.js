...
proto.update = function (data) {
    ...
    function toDataCoords(axis, coord, scale, calendar) {
        return coord.map((x) => axis.d2l(x, 0, calendar) * scale);
    }
    const positions = zip3(toDataCoords(layout.xaxis, data._meshX, scene.dataScale[0], data.xcalendar), toDataCoords(layout.yaxis, data._meshY, scene.dataScale[1], data.ycalendar), toDataCoords(layout.zaxis, data._meshZ, scene.dataScale[2], data.zcalendar));
...
function generateIsoMeshes(data) {
    ...
    function tryCreateTri(style, xyzv, abc, min, max, nPass) {
        ...
        const tryDrawTri = (style, xyzv, abc) => {
    ...
    function beginSection(style, i, j, k, min, max, distRatios, _previousResult) {
                ...
                if (slice.show && slice.fill) {
                    ...
                    if (slice.locations.length) {
                        for (let q = 0; q < slice.locations.length; q++) {
                            const near = findNearestOnAxis(slice.locations[q], (e === 'x') ? Xs :
                                (e === 'y') ? Ys : Zs);
                            if (near.distRatio === 0) {
