...
function createPreprocessor(config, emitter, testEntryPoint, bundlerMap, logger) {
    ...
    config.formatError = (0, format_error_1.createFormatError)(bundlerMap, basePath, config.formatError);
    let watcher = null;
    ...
    if (watchMode) {
        ...
        const onWatch = (0, utils_1.debounce)(() => {
    ...
    bundlerMap.on("done", ev => {
        ...
        if (singleBundle || pendingBundles.size === 0) {
            log.info(`Compiling done (${(0, utils_1.formatTime)(ev.endTime - start)})`);
        }
    });
    const buildSingleBundle = (0, utils_1.debounce)(() => {
